...
void main(int n)
{
  ...
  if (n > 6 && n <= 100) {
    ...
    // There's a bug with embedded fields in Jessie translation:
    // after allocationg aout1/aout2 ain1/ain2 is not valid!
    // The reason is that the new alloc_table is used (because of the inner field)
    // for which the alloc_extends predicate isn't added to the Why3 specification.
