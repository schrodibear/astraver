...
/*@ requires \valid(i);
    assigns d_find_alias(i)->d_lockref.count;
    ensures d_inc{Pre, Post}(d_find_alias(i));
    ensures \result == d_find_alias(i) && \valid(\result) && \result->d_inode == i || \result == \null;
 */
...
int i_getxattr(struct dentry *d, struct inode *i, const char *name, void **val, int f)
{
    ...
    *val=kmalloc(rc,GFP_KERNEL);
    if(!*val){ T("return ENOMEM"); rc=-ENOMEM; goto dput_fake_d; }
