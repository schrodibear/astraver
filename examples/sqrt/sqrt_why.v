...
Lemma sqrt_po_7 : 
  ...
  (y1: Z)
  (z1: Z)
  (Post18: `z1 > 0` /\ `y1 > 0` /\ z1 = `(Zdiv ((Zdiv x y1) + y1) 2)` /\
...
Lemma sqrt_po_11 : 
  ...
  (y0: Z)
  (z0: Z)
  (Post14: `z0 > 0` /\ `y0 > 0` /\ z0 = `(Zdiv ((Zdiv x y0) + y0) 2)` /\
           `x < (y0 + 1) * (y0 + 1)` /\ `x < (z0 + 1) * (z0 + 1)` /\
           `z0 >= y0`)
  `y0 * y0 <= x` /\ `x < (y0 + 1) * (y0 + 1)`.
Proof.
...
Definition sqrt := (* validation *)
  [x: Z]
    [Pre7: `x >= 0`]
      let (result, Post7) =
        let (result, Bool3) =
          let (result1, Post8) = (Z_eq_bool x `0`) in
          (exist_1 [result2: bool](if result2 then x = `0` else ~(x = `0`)) 
          result1 Post8) in
        (Cases (btest [result:bool](if result then x = `0` else ~(x = `0`)) result Bool3) of
        | (left Test6) =>
            let (result0, Post21) =
            ...
            `x < (result1 + 1) * (result1 + 1)` result0 Post21)
        | (right Test5) =>
            let (result0, Post9) =
              let (result0, Bool2) =
                let (result2, Post10) = (Z_le_gt_bool x `3`) in
                (exist_1 [result3: bool](if result3 then `x <= 3`
                                         else `x > 3`) result2
                Post10) in
              (Cases (btest [result0:bool](if result0 then `x <= 3`
                                           else `x > 3`) result0 Bool2) of
              | (left Test4) =>
                  let (result1, Post20) =
                    (exist_1 [result1: Z]`result1 * result1 <= x` /\
                  ...
                  `x < (result2 + 1) * (result2 + 1)` result1 Post20)
              | (right Test3) =>
                  let (result1, Post11) =
                    let (result1, Post1) = (exist_1 [result1: Z]result1 = x 
                      x (refl_equal ? x)) in
                    let (y0, result2, Post12) =
                      let Pre1 =
                        (sqrt_po_3 x Pre7 Test5 Test3 result1 Post1) in
                      let (result2, Post2) =
                        (exist_1 [result2: Z]result2 = `(Zdiv (x + 1) 2)` 
                        `(Zdiv (x + 1) 2)`
                        (refl_equal ? `(Zdiv (x + 1) 2)`)) in
                      let (y0, z0, result3, Post13) =
                        let (y0, z0, result3, Post14) =
                          ((((((((((well_founded_induction Z) (Zwf ZERO))
                                   (sqrt_po_4 x Pre7 Test5 Test3 result1
                                   Post1 Pre1 result2 Post2))
                                  [Variant1: Z](y0: Z)(z0: Z)(_: Variant1 =
                                                                 y0)(_: `
                                  z0 > 0` /\ `y0 > 0` /\
                                  z0 = `(Zdiv ((Zdiv x y0) + y0) 2)` /\
                                  `x < (y0 + 1) * (y0 + 1)` /\
                                  `x < (z0 + 1) * (z0 + 1)`)(sig_3 Z Z
                                  unit [y1:Z][z1:Z][result:unit](`z1 > 0` /\
                                  `y1 > 0` /\
                                  z1 = `(Zdiv ((Zdiv x y1) + y1) 2)` /\
                                  `x < (y1 + 1) * (y1 + 1)` /\
                                  `x < (z1 + 1) * (z1 + 1)` /\ `z1 >= y1`)))
                                 [Variant1: Z]
                                   [wf1: (Variant2: Z)(Pre2: (Zwf `0` Variant2 Variant1))(y0: Z)(z0: Z)(_: 
                                     Variant2 = y0)(_: `z0 > 0` /\
                                     `y0 > 0` /\
                                     z0 = `(Zdiv ((Zdiv x y0) + y0) 2)` /\
                                     `x < (y0 + 1) * (y0 + 1)` /\
                                     `x < (z0 + 1) * (z0 + 1)`)(sig_3 Z Z
                                     unit [y1:Z][z1:Z][result:unit](`
                                     z1 > 0` /\ `y1 > 0` /\
                                     z1 = `(Zdiv ((Zdiv x y1) + y1) 2)` /\
                                     `x < (y1 + 1) * (y1 + 1)` /\
                                     `x < (z1 + 1) * (z1 + 1)` /\
                                         ...
                                         [Pre6: Variant1 = y0]
                                           [Pre5: `z0 > 0` /\ `y0 > 0` /\
                                             ...
                                             let (result3, Bool1) =
                                               ...
                                               if result6 then `z0 < y0`
                                               else `z0 >= y0`) result5
                                               Post15) in
                                             (Cases (btest [result3:bool](
                                             if result3 then `z0 < y0`
                                             else `z0 >= y0`) result3 Bool1) of
                                             | (left Test2) =>
                                                 let (y1, z1, result4,
                                                   Post17) =
                                                   let (y1, z1, result4,
                                                     ...
                                                     (Zwf `0` y2 y0) 
                                                     y1 z1 result5
                                                     (sqrt_po_6 x Pre7 Test5
                                                     Test3 result1 Post1 Pre1
                                                     result2 Post2 Variant1
                                                     y0 z0 Pre6 Pre5 Test2 y1
                                                     Post3 Pre3 Pre4 z1
                                                     Post4)) in
                                                   ((((((wf1 y1)
                                                         (sqrt_po_7 x Pre7
                                             ...
                                             | (right Test1) =>
                                                 let (y1, z1, result4,
                                                   ...
                                                   `x < (z1 + 1) * (z1 + 1)` /\
                                                   `z1 >= y1` y0 z0 tt
                                                   (sqrt_po_9 x Pre7 Test5
                                                   Test3 result1 Post1 Pre1
                                                   result2 Post2 Variant1 y0
                                                   z0 Pre6 Pre5 Test1)) in
                                                 (exist_3 [y2: Z][z2: Z][result5: unit]`
                                                 z2 > 0` /\ `y2 > 0` /\
                                                 z2 =
                                                 `(Zdiv ((Zdiv x y2) + y2) 2)` /\
                                                 ...
                                                 `z2 >= y2` y1 z1 result4
                                                 Post16) end)) result1)
                               result1) result2) (refl_equal ? result1))
                            (sqrt_po_10 x Pre7 Test5 Test3 result1 Post1 Pre1
                            result2 Post2)) in
                        let (result4, Post19) =
                          (exist_1 [result4: Z]`result4 * result4 <= x` /\
                          `x < (result4 + 1) * (result4 + 1)` y0
                          (sqrt_po_11 x Pre7 Test5 Test3 result1 Post1 Pre1
                          result2 Post2 y0 z0 Post14)) in
                        (exist_3 [y1: Z][z1: Z][result5: Z]`result5 *
                                                            result5 <= x` /\
                        `x < (result5 + 1) * (result5 + 1)` y0 z0 result4
                        Post19) in
                      (exist_2 [y1: Z][result4: Z]`result4 * result4 <= x` /\
                      `x < (result4 + 1) * (result4 + 1)` y0 result3 Post13) in
                    (exist_1 [result3: Z]`result3 * result3 <= x` /\
                    `x < (result3 + 1) * (result3 + 1)` result2 Post12) in
                  (exist_1 [result2: Z]`result2 * result2 <= x` /\
                  `x < (result2 + 1) * (result2 + 1)` result1 Post11) end) in
            (exist_1 [result1: Z]`result1 * result1 <= x` /\
            `x < (result1 + 1) * (result1 + 1)` result0 Post9) end) in
      (exist_1 [result0: Z]`result0 * result0 <= x` /\
      `x < (result0 + 1) * (result0 + 1)` result Post7).
