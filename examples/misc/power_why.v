...
Definition power1 (* validation *)
  : (m: Z)(n: Z)(y: Z)(_: `n >= 0`)
    (sig_4 Z Z Z unit [m0: Z][n0: Z][y0: Z][result: unit]
     (`y0 = (Zpower x n)`))
  := [m: Z; n: Z; y: Z; Pre4: `n >= 0`]
       ...
       let (m1, n0, y1, result1, Post6) =
         (well_founded_induction Z (Zwf ZERO) (Zwf_well_founded `0`)
           [Variant1: Z](m1: Z)(n0: Z)(y1: Z)(_: Variant1 = n0)
           (_0: `(Zpower x n) = y1 * (Zpower m1 n0)` /\ `n0 >= 0`)
           (sig_4 Z Z Z unit [m2: Z][n1: Z][y2: Z][result1: unit]
            ((`(Zpower x n) = y2 * (Zpower m2 n1)` /\ `n1 >= 0`) /\
            `n1 <= 0`))
           [Variant1: Z; wf1: (Variant2: Z)
            (Pre1: (Zwf `0` Variant2 Variant1))(m1: Z)(n0: Z)(y1: Z)
            (_: Variant2 = n0)(_0: `(Zpower x n) = y1 * (Zpower m1 n0)` /\
            `n0 >= 0`)
            (sig_4 Z Z Z unit [m2: Z][n1: Z][y2: Z][result1: unit]
             ((`(Zpower x n) = y2 * (Zpower m2 n1)` /\ `n1 >= 0`) /\
             `n1 <= 0`));
            ...
            Pre2: `(Zpower x n) = y1 * (Zpower m1 n0)` /\ `n0 >= 0`]
                   ...
                   let (m2, n1, y2, result2, Post7) =
                     let (y2, result2, Post10) =
                       let (result2, Bool1) =
                         ...
                         (if result5 then (Zodd n0) else (Zeven n0)) 
                         result4 Post11) in
                       (Cases (btest
                            ...
                            (m = `m1 * m1` ->
                             ((n1:Z)
                              (n1 = (div2 n0) ->
                               ...
                               `n1 >= 0`) /\ (Zwf `0` n1 n0))))) y2
                           result3
                           (power1_po_1 n Pre4 m0 Post1 y0 Post2 Variant1 m1
                           n0 y1 Pre3 Pre2 Test4 Test3 y2 Post3))
                       | (right Test2) =>
                              ...
                              (m = `m1 * m1` ->
                               ((n1:Z)
                                (n1 = (div2 n0) ->
                                 ...
                                 `n1 >= 0`) /\ (Zwf `0` n1 n0))))) tt
                             (power1_po_2 n Pre4 m0 Post1 y0 Post2 Variant1
                             m1 n0 y1 Pre3 Pre2 Test4 Test2)) in
             ...
             | (right Test1) =>
                 let (m2, n1, y2, result2, Post6) = (exist_4 [m2: Z][n1: Z]
                   [y2: Z][result2: unit]
                   (`(Zpower x n) = y2 * (Zpower m2 n1)` /\ `n1 >= 0`) /\
                   `n1 <= 0` m1 n0 y1 tt (conj ? ? Pre2 Test1)) in
                 (exist_4 [m3: Z][n2: Z][y3: Z][result3: unit]
                 (`(Zpower x n) = y3 * (Zpower m3 n2)` /\ `n2 >= 0`) /\
