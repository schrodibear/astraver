...
Definition gcd1 (* validation *)
  : (a: Z)(b: Z)(_: `a > 0` /\ `b > 0`)
    (sig_1 Z [result: Z](`result = (gcd a b)`))
  := [a: Z; b: Z; Pre4: `a > 0` /\ `b > 0`]
       let (result, Post5) = (exist_1 [result: Z]result = a a
         (refl_equal ? a)) in
       let (x0, result0, Post8) =
         let (result0, Post4) = (exist_1 [result0: Z]result0 = b b
           (refl_equal ? b)) in
         let (x0, y0, result1, Post9) =
           let (x0, y0, result1, Post3) =
             (well_founded_induction Z (Zwf ZERO) (Zwf_well_founded `
               0`) [Variant1: Z](x0: Z)(y0: Z)(_: Variant1 = (max x0 y0))
               (_0: `0 < x0` /\ `0 < y0` /\ `(gcd x0 y0) = (gcd a b)`)
               ...
               [Variant1: Z; wf1: (Variant2: Z)
                ...
                `0 < y0` /\ `(gcd x0 y0) = (gcd a b)`]
                 let (result1, Bool2) =
                   ...
                   (if result4 then `x0 <> y0` else `x0 = y0`) result3
                   Post10) in
                 (Cases (btest
                       ...
                       let (x1, y1, result2, Post6) =
                         let (x1, y1, result2, Post6) =
                           ...
                           (Cases (btest
                                   ...
                                   result2 Bool1) of
                           | (left Test3) =>
                               let (x1, result3, Post1) =
                                 let (result3, Post1) = (exist_1 [result3: Z]
                                   result3 = `x0 - y0` `x0 - y0`
                                   (refl_equal ? `x0 - y0`)) in
                                 (exist_2 [x2: Z][result4: unit]
                                 x2 = `x0 - y0` result3 tt Post1) in
                               (exist_3 [x2: Z][y1: Z][result4: unit]
                               (`0 < x2` /\ `0 < y1` /\
                               `(gcd x2 y1) = (gcd a b)`) /\
                               (Zwf `0` (max x2 y1) (max x0 y0)) x1 y0
                               result3
                               (gcd1_po_1 a b Pre4 result Post5 result0 Post4
                               Variant1 x0 y0 Pre3 Pre2 Test4 Test3 x1 Post1))
                           | (right Test2) =>
                               let (y1, result3, Post2) =
                                 let (result3, Post2) = (exist_1 [result3: Z]
                                   result3 = `y0 - x0` `y0 - x0`
                                   (refl_equal ? `y0 - x0`)) in
                                 (exist_2 [y2: Z][result4: unit]
                                 y2 = `y0 - x0` result3 tt Post2) in
                               (exist_3 [x1: Z][y2: Z][result4: unit]
                               (`0 < x1` /\ `0 < y2` /\
                               `(gcd x1 y2) = (gcd a b)`) /\
                               (Zwf `0` (max x1 y2) (max x0 y0)) x0 y1
                               result3
                               (gcd1_po_2 a b Pre4 result Post5 result0 Post4
                               Variant1 x0 y0 Pre3 Pre2 Test4 Test2 y1 Post2)) end) in
                         (exist_3 [x2: Z][y2: Z][result3: unit](`0 < x2` /\
                         `0 < y2` /\ `(gcd x2 y2) = (gcd a b)`) /\
                         (Zwf `0` (max x2 y2) (max x0 y0)) x1 y1 result2
                         Post6) in
                       ((wf1 (max x1 y1)) (loop_variant_1 Pre3 Post6) 
                         x1 y1 (refl_equal ? (max x1 y1)) (proj1 ? ? Post6)) in
                     (exist_3 [x2: Z][y2: Z][result3: unit](`0 < x2` /\
                     `0 < y2` /\ `(gcd x2 y2) = (gcd a b)`) /\ `x2 = y2` 
                     x1 y1 result2 Post3)
                 | (right Test1) =>
                     let (x1, y1, result2, Post3) = (exist_3 [x1: Z][y1: Z]
                       [result2: unit](`0 < x1` /\ `0 < y1` /\
                       `(gcd x1 y1) = (gcd a b)`) /\ `x1 = y1` x0 y0 
                       tt (conj ? ? Pre2 Test1)) in
                     (exist_3 [x2: Z][y2: Z][result3: unit](`0 < x2` /\
                     `0 < y2` /\ `(gcd x2 y2) = (gcd a b)`) /\ `x2 = y2` 
           ...
           let (result2, Post12) = (exist_1 [result2: Z]
             `result2 = (gcd a b)` x0
             (gcd1_po_4 a b Pre4 result Post5 result0 Post4 x0 y0 Post3)) in
           (exist_3 [x1: Z][y1: Z][result3: Z]`result3 = (gcd a b)` x0 
           y0 result2 Post12) in
...
Definition gcd2 (* validation *)
  ...
  := [a: Z; b: Z; Pre5: `a >= 0` /\ `b >= 0`]
       let (result, Post6) = (exist_1 [result: Z]result = a a
         (refl_equal ? a)) in
       let (x0, result0, Post8) =
         let (result0, Post5) = (exist_1 [result0: Z]result0 = b b
           (refl_equal ? b)) in
         let (x0, y0, result1, Post9) =
           let (x0, y0, result1, Post4) =
             (well_founded_induction Z (Zwf ZERO) (Zwf_well_founded `
               0`) [Variant1: Z](x0: Z)(y0: Z)(_: Variant1 = y0)
               (_0: `0 <= x0` /\ `0 <= y0` /\ `(gcd x0 y0) = (gcd a b)`)
               (sig_3 Z Z unit [x1: Z][y1: Z][result1: unit]((`0 <= x1` /\
               ...
               [Variant1: Z; wf1: (Variant2: Z)
                ...
                `0 <= y0` /\ `(gcd x0 y0) = (gcd a b)`]
                 ...
                 (Cases (btest
                         [result1:bool](if result1 then `y0 <> 0`
                                        else `y0 = 0`)
                         result1 Bool1) of
                 | (left Test2) =>
                     let (x1, y1, result2, Post4) =
                       let (x1, y1, result2, Post7) =
                         let (x1, y1, result2, Post7) =
                           let Pre2 =
                             (gcd2_po_1 a b Pre5 result Post6 result0 Post5
                             Variant1 x0 y0 Pre4 Pre3 Test2) in
                           let (r, Post3) = (exist_1 [result2: Z]
                             result2 = (Zmod x0 y0) (Zmod x0 y0)
                             (refl_equal ? (Zmod x0 y0))) in
                           let (x1, y1, result2, Post7) =
                             let (x1, result2, Post1) =
                               let (result2, Post1) = (exist_1 [result2: Z]
                                 result2 = y0 y0 (refl_equal ? y0)) in
                               (exist_2 [x2: Z][result3: unit]x2 = y0 
                               result2 tt Post1) in
                             let (y1, result3, Post2) =
                               let (result3, Post2) = (exist_1 [result3: Z]
                                 result3 = r r (refl_equal ? r)) in
                               (exist_2 [y2: Z][result4: unit]y2 = r 
                               result3 tt Post2) in
                             (exist_3 [x2: Z][y2: Z][result4: unit]
                             (`0 <= x2` /\ `0 <= y2` /\
                             `(gcd x2 y2) = (gcd a b)`) /\ (Zwf `0` y2 y0) 
                             x1 y1 result3
                             (gcd2_po_2 a b Pre5 result Post6 result0 Post5
                             Variant1 x0 y0 Pre4 Pre3 Test2 Pre2 r Post3 x1
                             Post1 y1 Post2)) in
                           (exist_3 [x2: Z][y2: Z][result3: unit]
                           (`0 <= x2` /\ `0 <= y2` /\
                           `(gcd x2 y2) = (gcd a b)`) /\ (Zwf `0` y2 y0) 
                           x1 y1 result2 Post7) in
                         (exist_3 [x2: Z][y2: Z][result3: unit](`0 <= x2` /\
                         `0 <= y2` /\ `(gcd x2 y2) = (gcd a b)`) /\
                     ...
                     x1 y1 result2 Post4)
                 | (right Test1) =>
                     let (x1, y1, result2, Post4) = (exist_3 [x1: Z][y1: Z]
                       [result2: unit](`0 <= x1` /\ `0 <= y1` /\
           ...
           let (result2, Post11) = (exist_1 [result2: Z]
             `result2 = (gcd a b)` x0
             (gcd2_po_4 a b Pre5 result Post6 result0 Post5 x0 y0 Post4)) in
           (exist_3 [x1: Z][y1: Z][result3: Z]`result3 = (gcd a b)` x0 
           y0 result2 Post11) in
         (exist_2 [x1: Z][result2: Z]`result2 = (gcd a b)` x0 result1 Post9) in
       (exist_1 [result1: Z]`result1 = (gcd a b)` result0 Post8).
