       ...
       let (x0, z0, result0, I) =
         (well_founded_induction Z (Zwf ZERO) (Zwf_well_founded `0`)
           ...
           [Variant1: Z; wf1: (Variant2: Z)
            (Pre1: (Zwf `0` Variant2 Variant1))(x0: Z)(z0: Z)
            (_: Variant2 = z0)(I: `0 <= z0` /\ `x0 = x + (y - z0)`)
            (sig_3 Z Z unit [x1: Z][z1: Z][result0: unit]((`0 <= z1` /\
             `x1 = x + (y - z1)`) /\ `z1 <= 0`));
       ...
       (well_founded_induction Z (Zwf ZERO) (Zwf_well_founded `0`)
         ...
         [Variant1: Z; wf1: (Variant2: Z)(Pre1: (Zwf `0` Variant2 Variant1))
          ...
          Pre7: Variant1 = y0; Pre6: `y0 >= 0`]
           ...
           (Cases (btest
                   [result:bool](if result then `0 < y0` else `0 >= y0`)
                   result Bool3) of
           | (left Test2) =>
...
Definition mult1 (* validation *)
  ...
  := [y: Z; x: Z; Pre6: `x >= 0` /\ `y >= 0`]
       ...
       let (x0, z0, result0, Post5) =
         ...
         let (x0, z0, result0, Post6) =
           ...
           let (x1, z0, result1, I) =
             (well_founded_induction Z (Zwf ZERO) (Zwf_well_founded `
               0`) [Variant1: Z](x1: Z)(z0: Z)(_: Variant1 = z0)
               (I: `0 <= z0` /\ `x1 = x * (y - z0)`)
               ...
               [Variant1: Z; wf1: (Variant2: Z)
                ...
                `x1 = x * (y - z0)`]
                 ...
                 (Cases (btest
                         [result1:bool](if result1 then `z0 > 0`
                                        else `z0 <= 0`)
                         result1 Bool1) of
                 | (left Test2) =>
                     let (x2, z1, result2, I0) =
                       let (x2, z1, result2, I0) =
                         ...
                         let (x2, result2, Post8) =
                           ...
                           (exist_2 [x3: Z][result5: unit]
                           `x3 = x1 + savex` x2 result4 Post9) in
                         let (z1, result3, Post2) =
                           let (result3, Post2) = (exist_1 [result3: Z]
                             result3 = `z0 - 1` `z0 - 1`
                             (refl_equal ? `z0 - 1`)) in
                           (exist_2 [z2: Z][result4: unit]
                           z2 = `z0 - 1` result3 tt Post2) in
                         (exist_3 [x3: Z][z2: Z][result4: unit](`0 <= z2` /\
                         `x3 = x * (y - z2)`) /\ (Zwf `0` z2 z0) x2 z1
                         result3
                         (mult1_po_2 y x Pre6 result Post4 savex Post3 x0
                         Post1 Variant1 x1 z0 Pre5 I Test2 Pre4 x2 Post8 z1
                         Post2)) in
         ...
         (exist_3 [x1: Z][z1: Z][result1: unit]`x1 = x * y` x0 z0 result0
         Post6) in
