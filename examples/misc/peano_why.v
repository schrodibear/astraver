  ...
  [y: Z; x: Z; Pre5: `y >= 0`]
    (well_founded_induction Z (Zwf ZERO) (rec_add1_po_1 y Pre5) [Variant1: Z]
      ...
      [Variant1: Z; wf1: (Variant2: Z)(Pre1: (Zwf `0` Variant2 Variant1))
       ...
       (sig_2 Z unit [x1:Z][result:unit](x1 = `x0 + y`)); y0: Z; x0: Z;
       Pre4: Variant1 = y0; Pre3: `y0 >= 0`]
        let (result, Bool1) =
          let (result1, Post3) = (Z_lt_ge_bool `0` y0) in
          (exist_1 [result2: bool]
          (if result2 then `0 < y0` else `0 >= y0`) result1 Post3) in
        ...
        | (left Test2) =>
            let (x1, result0, Post5) =
              ...
              let (x2, result1, Post6) =
                let Pre2 =
                  (rec_add1_po_2 y Pre5 Variant1 y0 x0 Pre4 Pre3 Test2 x1
                  Post1) in
                let (x2, result3, Post7) =
                  ((wf1 `y0 - 1`)
                    (rec_add1_po_3 y Pre5 Variant1 y0 x0 Pre4 Pre3 Test2 x1
                    Post1 Pre2) `y0 - 1` x1 (refl_equal ? `y0 - 1`) Pre2 
                    ) in
                (exist_2 [x3: Z][result4: unit]x3 = `x1 + (y0 - 1)` x2 
                result3 Post7) in
              (exist_2 [x3: Z][result2: unit]x3 = `x0 + y0` x2 result1
              (rec_add1_po_4 y Pre5 Variant1 y0 x0 Pre4 Pre3 Test2 x1 Post1 x2
              Post6)) in
            (exist_2 [x2: Z][result1: unit]x2 = `x0 + y0` x1 result0 Post5)
        | (right Test1) =>
            let (result0, Post4) = (exist_1 [result0: unit]x0 = `x0 + y0` 
              tt (rec_add1_po_5 y Pre5 Variant1 y0 x0 Pre4 Pre3 Test1)) in
            (exist_2 [x1: Z][result1: unit]x1 = `x0 + y0` x0 result0 Post4) end)
      ...
      let (x0, z0, result1, Post6) =
        ...
        let (x1, z0, result2, I) =
          (well_founded_induction Z (Zwf ZERO)
            (mult1_po_1 y x Pre4 result Post1 result0 Post2 x0 Post3)
            ...
            [Variant1: Z; wf1: (Variant2: Z)
             ...
             x1 = `x * (y - z0)`]
              let (result2, Bool1) =
                let (result4, Post7) = (Z_gt_le_bool z0 `0`) in
                (exist_1 [result5: bool]
                (if result5 then `z0 > 0` else `z0 <= 0`) result4 Post7) in
              (Cases (btest
                      [result2:bool](if result2 then `z0 > 0` else `z0 <= 0`)
                      result2 Bool1) of
              | (left Test2) =>
                  let (x2, z1, result3, I) =
                    let (x2, z1, result3, I0) =
                      let (x2, result3, Post8) =
                        let Pre2 =
                          (mult1_po_2 y x Pre4 result Post1 result0 Post2 x0
                          Post3 Variant1 x1 z0 Pre3 I Test2) in
                        let (x2, result5, Post9) = (add1 result0 x1 Pre2) in
                        (exist_2 [x3: Z][result6: unit]x3 = `x1 + result0` 
                        x2 result5 Post9) in
                      let (z1, result4, Post4) =
                        let (result4, Post4) = (exist_1 [result4: Z]
                          result4 = `z0 - 1` `z0 - 1`
                          (refl_equal ? `z0 - 1`)) in
                        (exist_2 [z2: Z][result5: unit]z2 = `z0 - 1` 
                        result4 tt Post4) in
                      (exist_3 [x3: Z][z2: Z][result5: unit]`0 <= z2` /\
                      x3 = `x * (y - z2)` /\ (Zwf `0` z2 z0) x2 z1 result4
                      (mult1_po_3 y x Pre4 result Post1 result0 Post2 x0
                      Post3 Variant1 x1 z0 Pre3 I Test2 x2 Post8 z1 Post4)) in
                    ((wf1 z1)
                      (mult1_po_4 y x Pre4 result Post1 result0 Post2 x0
                      Post3 Variant1 x1 z0 Pre3 I Test2 x2 z1 I0) x2 
                      z1 (refl_equal ? z1)
                      (mult1_po_5 y x Pre4 result Post1 result0 Post2 x0
                      Post3 Variant1 x1 z0 Pre3 I Test2 x2 z1 I0)) in
                  (exist_3 [x3: Z][z2: Z][result4: unit]`0 <= z2` /\
                  x3 = `x * (y - z2)` /\ `z2 <= 0` x2 z1 result3 I)
              | (right Test1) =>
                  let (x2, z1, result3, I) = (exist_3 [x2: Z][z1: Z]
                    [result3: unit]`0 <= z1` /\ x2 = `x * (y - z1)` /\
                    `z1 <= 0` x1 z0 tt
                    (mult1_po_6 y x Pre4 result Post1 result0 Post2 x0 Post3
                    Variant1 x1 z0 Pre3 I Test1)) in
                  (exist_3 [x3: Z][z2: Z][result4: unit]`0 <= z2` /\
                  x3 = `x * (y - z2)` /\ `z2 <= 0` x2 z1 result3 I) end)
            result x0 result (refl_equal ? result)
            (mult1_po_7 y x Pre4 result Post1 result0 Post2 x0 Post3)) in
        (exist_3 [x2: Z][z1: Z][result3: unit]x2 = `x * y` x1 z0 result2
        (mult1_po_8 y x Pre4 result Post1 result0 Post2 x0 Post3 x1 z0 I)) in
      (exist_3 [x1: Z][z1: Z][result2: unit]x1 = `x * y` x0 z0 result1 Post6) in
