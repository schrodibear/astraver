...
Definition bresenham (* validation *)
  : (e: Z)(x: Z)(y: Z)(tuple_4 Z Z Z unit)
  := [e: Z; x: Z; y: Z]
       let (x0, result, Post1) =
       ...
       let (e1, x1, y1, result2, Post8) =
         (well_founded_induction Z (Zwf ZERO) (Zwf_well_founded `0`)
           [Variant1: Z](e1: Z)(x1: Z)(y1: Z)(_: Variant1 = `x2 + 1 - x1`)
           (_0: (`0 <= x1` /\ `x1 <= x2 + 1`) /\ (invariant x1 y1 e1))
           (sig_4 Z Z Z unit [e2: Z][x3: Z][y3: Z][result2: unit]
            (((`0 <= x3` /\ `x3 <= x2 + 1`) /\ (invariant x3 y3 e2)) /\
            `x3 > x2`))
           [Variant1: Z; wf1: (Variant2: Z)
            (Pre1: (Zwf `0` Variant2 Variant1))(e1: Z)(x1: Z)(y1: Z)
            (_: Variant2 = `x2 + 1 - x1`)(_0: (`0 <= x1` /\
             ...
             | (left Test4) =>
                 let (e2, x3, y3, result3, Post8) =
                   let (e2, x3, y3, result3, Post9) =
                     let Pre2 =
                       (bresenham_po_1 x0 Post1 y0 Post2 e0 Post3 Variant1 e1
                       x1 y1 Pre4 Pre3 Test4) in
                     let (e2, y3, result3, Post12) =
                       ...
                       | (left Test3) =>
                           ...
                           y1 result4
                           (bresenham_po_2 x0 Post1 y0 Post2 e0 Post3
                           Variant1 e1 x1 y1 Pre4 Pre3 Test4 Pre2 Test3 e2
                           Post4))
                       | (right Test2) =>
                           let (e2, y3, result4, Post14) =
                             ...
                             ((x:Z)
                              (x = `x1 + 1` -> ((`0 <= x` /\
                               `x <= x2 + 1`) /\ (invariant x y4 e3)) /\
                               (Zwf `0` `x2 + 1 - x` `x2 + 1 - x1`))) 
                             e2 y3 result5
                             (bresenham_po_3 x0 Post1 y0 Post2 e0 Post3
                             Variant1 e1 x1 y1 Pre4 Pre3 Test4 Pre2 Test2 y3
                             Post5 e2 Post6)) in
