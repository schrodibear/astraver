...
Lemma binary_search_po_1 : 
  ...
  (Pre5: `1 <= l1` /\ `u1 <= (array_length t) - 1` /\ (`0 <= p1` /\
         `p1 <= (array_length t) - 1`) /\
...
Definition binary_search (* validation *)
  ...
  := [l: Z; m: Z; p: Z; t: (array Z); u: Z; Pre7: `(array_length t) >= 1` /\
      (sorted_array t `1` `(array_length t) - 1`)]
       let (l0, result, Post1) =
       ...
       let (l1, m0, p1, u1, result2, Post9) =
         (well_founded_induction Z (Zwf ZERO) (Zwf_well_founded `0`)
             ...
             | (left Test6) =>
                 let (l2, m1, p2, u2, result3, Post9) =
                   let (l2, m1, p2, u2, result3, Post10) =
                       ...
                       | (right Test4) =>
                           let (l2, p2, u2, result5, Post10) =
                             let (result5, Bool1) =
                               let result6 =
                                 let Pre3 =
                                   (binary_search_po_4 t Pre7 l0 Post1 u0
                                   Post2 p0 Post3 Variant1 l1 p1 u1 Pre6 Pre5
                                   Test6 m1 Post4 Pre4 Test4) in
                             ...
                             | (left Test3) =>
                                 ...
                                 p0 Post3 Variant1 l1 p1 u1 Pre6 Pre5 Test6
                                 m1 Post4 Pre4 Test4 Test3 u2 Post6))
                             | (right Test2) =>
                                 let (l2, p2, result6, Post10) =
                                   ...
                                   l2 p2 result7
                                   (binary_search_po_6 t Pre7 l0 Post1 u0
                                   Post2 p0 Post3 Variant1 l1 p1 u1 Pre6 Pre5
                                   Test6 m1 Post4 Pre4 Test4 Test2 p2 Post7
                                   l2 Post8)) in
                     ...
                     result4 Post10) in
                   ((wf1 `2 + u2 - l2`) (loop_variant_1 Pre6 Post10) 
             ...
             | (right Test1) =>
                 let (l2, m1, p2, u2, result3, Post9) = (exist_5 [l2: Z]
                   ...
                   ((`p2 = 0` ->
                     ((In t `1` `(array_length t) - 1`) -> (In t l2 u2)))) /\
                   ((`p2 > 0` -> `(access t p2) = v`))) /\ `l2 > u2` 
                   l1 m0 p1 u1 tt (conj ? ? Pre5 Test1)) in
                 (exist_5 [l3: Z][m2: Z][p3: Z][u3: Z][result4: unit]
                 (`1 <= l3` /\ `u3 <= (array_length t) - 1` /\ (`0 <= p3` /\
                 `p3 <= (array_length t) - 1`) /\
                 ((`p3 = 0` ->
           ...
           (refl_equal ? `2 + u0 - l0`)
           (binary_search_po_7 t Pre7 l0 Post1 u0 Post2 p0 Post3)) in
       ...
       (binary_search_po_8 t Pre7 l0 Post1 u0 Post2 p0 Post3 l1 p1 u1 Post9)).
