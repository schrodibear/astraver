(* This file was originally generated by why.
   It can be modified; only the generated parts will be overwritten. *)

Require Why.
Require words.
Require Omega.
Require Sumbool.

(*Why*) Parameter n1 : Z.
Axiom n1_non_negative : `0 <= n1`.

(*Why*) Parameter n2 : Z.
Axiom n2_non_negative : `0 <= n2`.

Tactic Definition Omega' := 
  Generalize n1_non_negative; 
  Generalize n2_non_negative; 
  Abstract Omega.

Definition min_suffix :=
  [w1:(array n1 A)][w2:(array n2 A)][i,j,n:Z]
  (min_dist (suffix w1 i) (suffix w2 j) n).

Definition test_char := [a,b:A](bool_of_sumbool (A_eq_dec a b)).

Lemma distance_po_1 : 
  (i0: Z)
  (Post1: i0 = `0`)
  (well_founded ? (Zwf ZERO)).
Proof.
Auto with *.
Save.

Lemma distance_po_2 : 
  (i0: Z)
  (Post1: i0 = `0`)
  (Variant1: Z)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Pre4: Variant1 = `n2 + 1 - i1`)
  (Pre3: `0 <= i1` /\ `i1 <= n2 + 1` /\
         ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)))
  (Test2: `i1 <= n2`)
  (result1: Z)
  (Post2: (store t0 i1 result1) = (store t0 i1 `n2 - i1`))
  `0 <= i1` /\ `i1 < n2 + 1`.
Proof.
Intuition.
Save.

Lemma distance_po_3 : 
  (i0: Z)
  (Post1: i0 = `0`)
  (Variant1: Z)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Pre4: Variant1 = `n2 + 1 - i1`)
  (Pre3: `0 <= i1` /\ `i1 <= n2 + 1` /\
         ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)))
  (Test2: `i1 <= n2`)
  (t1: (array `n2 + 1` Z))
  (Post2: t1 = (store t0 i1 `n2 - i1`))
  (i2: Z)
  (Post3: i2 = `i1 + 1`)
  `0 <= i2` /\ `i2 <= n2 + 1` /\
  ((j:Z) (`0 <= j` /\ `j < i2` -> (access t1 j) = `n2 - j`)) /\
  (Zwf `0` `n2 + 1 - i2` `n2 + 1 - i1`).
Proof.
Intuition.
Rewrite Post2.
ArrayAccess i1 j Hij; Try Omega'.
Apply H2; Omega'.
Unfold Zwf; Omega'.
Save.

Lemma distance_po_4 : 
  (i0: Z)
  (Post1: i0 = `0`)
  (Variant1: Z)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Pre4: Variant1 = `n2 + 1 - i1`)
  (Pre3: `0 <= i1` /\ `i1 <= n2 + 1` /\
         ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)))
  (Test2: `i1 <= n2`)
  (i2: Z)
  (t1: (array `n2 + 1` Z))
  (Post19: `0 <= i2` /\ `i2 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i2` -> (access t1 j) = `n2 - j`)) /\
           (Zwf `0` `n2 + 1 - i2` `n2 + 1 - i1`))
  (Zwf `0` `n2 + 1 - i2` Variant1).
Proof.
Intuition.
Rewrite Pre4; Assumption.
Save.

Lemma distance_po_5 : 
  (i0: Z)
  (Post1: i0 = `0`)
  (Variant1: Z)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Pre4: Variant1 = `n2 + 1 - i1`)
  (Pre3: `0 <= i1` /\ `i1 <= n2 + 1` /\
         ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)))
  (Test2: `i1 <= n2`)
  (i2: Z)
  (t1: (array `n2 + 1` Z))
  (Post19: `0 <= i2` /\ `i2 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i2` -> (access t1 j) = `n2 - j`)) /\
           (Zwf `0` `n2 + 1 - i2` `n2 + 1 - i1`))
  `0 <= i2` /\ `i2 <= n2 + 1` /\
  ((j:Z) (`0 <= j` /\ `j < i2` -> (access t1 j) = `n2 - j`)).
Proof.
Intuition.
Save.

Lemma distance_po_6 : 
  (i0: Z)
  (Post1: i0 = `0`)
  (Variant1: Z)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Pre4: Variant1 = `n2 + 1 - i1`)
  (Pre3: `0 <= i1` /\ `i1 <= n2 + 1` /\
         ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)))
  (Test1: `i1 > n2`)
  `0 <= i1` /\ `i1 <= n2 + 1` /\
  ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)) /\ `i1 > n2`.
Proof.
Intuition.
Save.

Lemma distance_po_7 : 
  (t: (array `n2 + 1` Z))
  (i0: Z)
  (Post1: i0 = `0`)
  `0 <= i0` /\ `i0 <= n2 + 1` /\
  ((j:Z) (`0 <= j` /\ `j < i0` -> (access t j) = `n2 - j`)).
Proof.
Intuition.
Omega'.
Save.

Lemma distance_po_8 : 
  (i0: Z)
  (Post1: i0 = `0`)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Post15: `0 <= i1` /\ `i1 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)) /\
           `i1 > n2`)
  (i2: Z)
  (Post4: i2 = `n1 - 1`)
  (well_founded ? (Zwf ZERO)).
Proof.
Intuition.
Save.

Lemma distance_po_9 : 
  (w1: (array n1 A))
  (w2: (array n2 A))
  (i0: Z)
  (Post1: i0 = `0`)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Post15: `0 <= i1` /\ `i1 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)) /\
           `i1 > n2`)
  (i2: Z)
  (Post4: i2 = `n1 - 1`)
  (Variant3: Z)
  (i3: Z)
  (t1: (array `n2 + 1` Z))
  (Pre20: Variant3 = `i3 + 1`)
  (Pre19: `(-1) <= i3` /\ `i3 <= n1 - 1` /\
          ((j:Z)
           (`0 <= j` /\ `j <= n2` ->
            (min_suffix w1 w2 `i3 + 1` j (access t1 j)))))
  (Test8: `i3 >= 0`)
  `0 <= n2` /\ `n2 < n2 + 1`.
Proof.
Intuition.
Omega'.
Save.

Lemma distance_po_10 : 
  (w1: (array n1 A))
  (w2: (array n2 A))
  (i0: Z)
  (Post1: i0 = `0`)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Post15: `0 <= i1` /\ `i1 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)) /\
           `i1 > n2`)
  (i2: Z)
  (Post4: i2 = `n1 - 1`)
  (Variant3: Z)
  (i3: Z)
  (t1: (array `n2 + 1` Z))
  (Pre20: Variant3 = `i3 + 1`)
  (Pre19: `(-1) <= i3` /\ `i3 <= n1 - 1` /\
          ((j:Z)
           (`0 <= j` /\ `j <= n2` ->
            (min_suffix w1 w2 `i3 + 1` j (access t1 j)))))
  (Test8: `i3 >= 0`)
  (Pre6: `0 <= n2` /\ `n2 < n2 + 1`)
  (old1: Z)
  (Post5: old1 = (access t1 n2))
  (Pre7: `0 <= n2` /\ `n2 < n2 + 1`)
  (t2: (array `n2 + 1` Z))
  (Post6: t2 = (store t1 n2 `(access t1 n2) + 1`))
  (j1: Z)
  (Post7: j1 = `n2 - 1`)
  (well_founded ? (Zwf ZERO)).
Proof.
Auto with *.
Save.

Lemma distance_po_11 : 
  (w1: (array n1 A))
  (w2: (array n2 A))
  (i0: Z)
  (Post1: i0 = `0`)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Post15: `0 <= i1` /\ `i1 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)) /\
           `i1 > n2`)
  (i2: Z)
  (Post4: i2 = `n1 - 1`)
  (Variant3: Z)
  (i3: Z)
  (t1: (array `n2 + 1` Z))
  (Pre20: Variant3 = `i3 + 1`)
  (Pre19: `(-1) <= i3` /\ `i3 <= n1 - 1` /\
          ((j:Z)
           (`0 <= j` /\ `j <= n2` ->
            (min_suffix w1 w2 `i3 + 1` j (access t1 j)))))
  (Test8: `i3 >= 0`)
  (Pre6: `0 <= n2` /\ `n2 < n2 + 1`)
  (old1: Z)
  (Post5: old1 = (access t1 n2))
  (Pre7: `0 <= n2` /\ `n2 < n2 + 1`)
  (t2: (array `n2 + 1` Z))
  (Post6: t2 = (store t1 n2 `(access t1 n2) + 1`))
  (j1: Z)
  (Post7: j1 = `n2 - 1`)
  (Variant5: Z)
  (j2: Z)
  (old2: Z)
  (t3: (array `n2 + 1` Z))
  (Pre18: Variant5 = `j2 + 1`)
  (Pre17: `(-1) <= j2` /\ `j2 <= n2 - 1` /\
          ((k:Z)
           (`j2 < k` /\ `k <= n2` -> (min_suffix w1 w2 i3 k (access t3 k)))) /\
          ((k:Z)
           (`0 <= k` /\ `k <= j2` ->
            (min_suffix w1 w2 `i3 + 1` k (access t3 k)))) /\
          (min_suffix w1 w2 `i3 + 1` `j2 + 1` old2))
  (Test7: `j2 >= 0`)
  (result7: Z)
  (Post8: result7 = old2)
  `0 <= j2` /\ `j2 < n2 + 1`.
Proof.
Intuition.
Save.

Lemma distance_po_12 : 
  (w1: (array n1 A))
  (w2: (array n2 A))
  (i0: Z)
  (Post1: i0 = `0`)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Post15: `0 <= i1` /\ `i1 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)) /\
           `i1 > n2`)
  (i2: Z)
  (Post4: i2 = `n1 - 1`)
  (Variant3: Z)
  (i3: Z)
  (t1: (array `n2 + 1` Z))
  (Pre20: Variant3 = `i3 + 1`)
  (Pre19: `(-1) <= i3` /\ `i3 <= n1 - 1` /\
          ((j:Z)
           (`0 <= j` /\ `j <= n2` ->
            (min_suffix w1 w2 `i3 + 1` j (access t1 j)))))
  (Test8: `i3 >= 0`)
  (Pre6: `0 <= n2` /\ `n2 < n2 + 1`)
  (old1: Z)
  (Post5: old1 = (access t1 n2))
  (Pre7: `0 <= n2` /\ `n2 < n2 + 1`)
  (t2: (array `n2 + 1` Z))
  (Post6: t2 = (store t1 n2 `(access t1 n2) + 1`))
  (j1: Z)
  (Post7: j1 = `n2 - 1`)
  (Variant5: Z)
  (j2: Z)
  (old2: Z)
  (t3: (array `n2 + 1` Z))
  (Pre18: Variant5 = `j2 + 1`)
  (Pre17: `(-1) <= j2` /\ `j2 <= n2 - 1` /\
          ((k:Z)
           (`j2 < k` /\ `k <= n2` -> (min_suffix w1 w2 i3 k (access t3 k)))) /\
          ((k:Z)
           (`0 <= k` /\ `k <= j2` ->
            (min_suffix w1 w2 `i3 + 1` k (access t3 k)))) /\
          (min_suffix w1 w2 `i3 + 1` `j2 + 1` old2))
  (Test7: `j2 >= 0`)
  (result7: Z)
  (Post8: result7 = old2)
  (Pre10: `0 <= j2` /\ `j2 < n2 + 1`)
  (old3: Z)
  (Post9: old3 = (access t3 j2))
  `0 <= j2` /\ `j2 < n2`.
Proof.
Intuition.
Save.

Lemma distance_po_13 : 
  (w1: (array n1 A))
  (w2: (array n2 A))
  (i0: Z)
  (Post1: i0 = `0`)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Post15: `0 <= i1` /\ `i1 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)) /\
           `i1 > n2`)
  (i2: Z)
  (Post4: i2 = `n1 - 1`)
  (Variant3: Z)
  (i3: Z)
  (t1: (array `n2 + 1` Z))
  (Pre20: Variant3 = `i3 + 1`)
  (Pre19: `(-1) <= i3` /\ `i3 <= n1 - 1` /\
          ((j:Z)
           (`0 <= j` /\ `j <= n2` ->
            (min_suffix w1 w2 `i3 + 1` j (access t1 j)))))
  (Test8: `i3 >= 0`)
  (Pre6: `0 <= n2` /\ `n2 < n2 + 1`)
  (old1: Z)
  (Post5: old1 = (access t1 n2))
  (Pre7: `0 <= n2` /\ `n2 < n2 + 1`)
  (t2: (array `n2 + 1` Z))
  (Post6: t2 = (store t1 n2 `(access t1 n2) + 1`))
  (j1: Z)
  (Post7: j1 = `n2 - 1`)
  (Variant5: Z)
  (j2: Z)
  (old2: Z)
  (t3: (array `n2 + 1` Z))
  (Pre18: Variant5 = `j2 + 1`)
  (Pre17: `(-1) <= j2` /\ `j2 <= n2 - 1` /\
          ((k:Z)
           (`j2 < k` /\ `k <= n2` -> (min_suffix w1 w2 i3 k (access t3 k)))) /\
          ((k:Z)
           (`0 <= k` /\ `k <= j2` ->
            (min_suffix w1 w2 `i3 + 1` k (access t3 k)))) /\
          (min_suffix w1 w2 `i3 + 1` `j2 + 1` old2))
  (Test7: `j2 >= 0`)
  (result7: Z)
  (Post8: result7 = old2)
  (Pre10: `0 <= j2` /\ `j2 < n2 + 1`)
  (old3: Z)
  (Post9: old3 = (access t3 j2))
  (Pre11: `0 <= j2` /\ `j2 < n2`)
  `0 <= i3` /\ `i3 < n1`.
Proof.
Intuition.
Save.

Lemma distance_po_14 : 
  (w1: (array n1 A))
  (w2: (array n2 A))
  (i0: Z)
  (Post1: i0 = `0`)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Post15: `0 <= i1` /\ `i1 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)) /\
           `i1 > n2`)
  (i2: Z)
  (Post4: i2 = `n1 - 1`)
  (Variant3: Z)
  (i3: Z)
  (t1: (array `n2 + 1` Z))
  (Pre20: Variant3 = `i3 + 1`)
  (Pre19: `(-1) <= i3` /\ `i3 <= n1 - 1` /\
          ((j:Z)
           (`0 <= j` /\ `j <= n2` ->
            (min_suffix w1 w2 `i3 + 1` j (access t1 j)))))
  (Test8: `i3 >= 0`)
  (Pre6: `0 <= n2` /\ `n2 < n2 + 1`)
  (old1: Z)
  (Post5: old1 = (access t1 n2))
  (Pre7: `0 <= n2` /\ `n2 < n2 + 1`)
  (t2: (array `n2 + 1` Z))
  (Post6: t2 = (store t1 n2 `(access t1 n2) + 1`))
  (j1: Z)
  (Post7: j1 = `n2 - 1`)
  (Variant5: Z)
  (j2: Z)
  (old2: Z)
  (t3: (array `n2 + 1` Z))
  (Pre18: Variant5 = `j2 + 1`)
  (Pre17: `(-1) <= j2` /\ `j2 <= n2 - 1` /\
          ((k:Z)
           (`j2 < k` /\ `k <= n2` -> (min_suffix w1 w2 i3 k (access t3 k)))) /\
          ((k:Z)
           (`0 <= k` /\ `k <= j2` ->
            (min_suffix w1 w2 `i3 + 1` k (access t3 k)))) /\
          (min_suffix w1 w2 `i3 + 1` `j2 + 1` old2))
  (Test7: `j2 >= 0`)
  (result7: Z)
  (Post8: result7 = old2)
  (Pre10: `0 <= j2` /\ `j2 < n2 + 1`)
  (old3: Z)
  (Post9: old3 = (access t3 j2))
  (Test6: (access w1 i3) = (access w2 j2))
  (t4: (array `n2 + 1` Z))
  (Post11: t4 = (store t3 j2 result7))
  ((j:Z)
   (j = `j2 - 1` -> `(-1) <= j` /\ `j <= n2 - 1` /\
    ((k:Z) (`j < k` /\ `k <= n2` -> (min_suffix w1 w2 i3 k (access t4 k)))) /\
    ((k:Z)
     (`0 <= k` /\ `k <= j` -> (min_suffix w1 w2 `i3 + 1` k (access t4 k)))) /\
    (min_suffix w1 w2 `i3 + 1` `j + 1` old3) /\ (Zwf `0` `j + 1` `j2 + 1`))).
Proof.
Intuition.
Unfold min_suffix.
Rewrite Post11.
ArrayAccess j2 k Hj2k.
  (* j2=k *)
  Rewrite (suffix_is_cons ? w1 i3); [ Idtac | Omega' ].
  Rewrite (suffix_is_cons ? w2 k); [ Idtac | Omega' ].
  Rewrite Test6. Rewrite <- Hj2k.
  Apply min_dist_equal.
  Rewrite Post8; Assumption.
  Omega'.
  (* j2<>k *)
  Unfold min_suffix in H11.
  Apply H11; Omega'.
  Omega'.
  Omega'.
Unfold min_suffix.
Rewrite Post11.
Rewrite store_def_2.
Unfold min_suffix in H13; Apply H13; Omega'.
Omega'.
Omega'.
Omega'.
Replace `j+1` with j2; [ Idtac | Omega' ].
Rewrite Post9.
Unfold min_suffix; Unfold min_suffix in H13; Apply H13; Omega'.
Unfold Zwf; Omega'.
Save.

Lemma distance_po_15 : 
  (w1: (array n1 A))
  (w2: (array n2 A))
  (i0: Z)
  (Post1: i0 = `0`)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Post15: `0 <= i1` /\ `i1 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)) /\
           `i1 > n2`)
  (i2: Z)
  (Post4: i2 = `n1 - 1`)
  (Variant3: Z)
  (i3: Z)
  (t1: (array `n2 + 1` Z))
  (Pre20: Variant3 = `i3 + 1`)
  (Pre19: `(-1) <= i3` /\ `i3 <= n1 - 1` /\
          ((j:Z)
           (`0 <= j` /\ `j <= n2` ->
            (min_suffix w1 w2 `i3 + 1` j (access t1 j)))))
  (Test8: `i3 >= 0`)
  (Pre6: `0 <= n2` /\ `n2 < n2 + 1`)
  (old1: Z)
  (Post5: old1 = (access t1 n2))
  (Pre7: `0 <= n2` /\ `n2 < n2 + 1`)
  (t2: (array `n2 + 1` Z))
  (Post6: t2 = (store t1 n2 `(access t1 n2) + 1`))
  (j1: Z)
  (Post7: j1 = `n2 - 1`)
  (Variant5: Z)
  (j2: Z)
  (old2: Z)
  (t3: (array `n2 + 1` Z))
  (Pre18: Variant5 = `j2 + 1`)
  (Pre17: `(-1) <= j2` /\ `j2 <= n2 - 1` /\
          ((k:Z)
           (`j2 < k` /\ `k <= n2` -> (min_suffix w1 w2 i3 k (access t3 k)))) /\
          ((k:Z)
           (`0 <= k` /\ `k <= j2` ->
            (min_suffix w1 w2 `i3 + 1` k (access t3 k)))) /\
          (min_suffix w1 w2 `i3 + 1` `j2 + 1` old2))
  (Test7: `j2 >= 0`)
  (result7: Z)
  (Post8: result7 = old2)
  (Pre10: `0 <= j2` /\ `j2 < n2 + 1`)
  (old3: Z)
  (Post9: old3 = (access t3 j2))
  (Test5: ~(access w1 i3) = (access w2 j2))
  `0 <= j2 + 1` /\ `j2 + 1 < n2 + 1`.
Proof.
Intuition.
Save.

Lemma distance_po_16 : 
  (w1: (array n1 A))
  (w2: (array n2 A))
  (i0: Z)
  (Post1: i0 = `0`)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Post15: `0 <= i1` /\ `i1 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)) /\
           `i1 > n2`)
  (i2: Z)
  (Post4: i2 = `n1 - 1`)
  (Variant3: Z)
  (i3: Z)
  (t1: (array `n2 + 1` Z))
  (Pre20: Variant3 = `i3 + 1`)
  (Pre19: `(-1) <= i3` /\ `i3 <= n1 - 1` /\
          ((j:Z)
           (`0 <= j` /\ `j <= n2` ->
            (min_suffix w1 w2 `i3 + 1` j (access t1 j)))))
  (Test8: `i3 >= 0`)
  (Pre6: `0 <= n2` /\ `n2 < n2 + 1`)
  (old1: Z)
  (Post5: old1 = (access t1 n2))
  (Pre7: `0 <= n2` /\ `n2 < n2 + 1`)
  (t2: (array `n2 + 1` Z))
  (Post6: t2 = (store t1 n2 `(access t1 n2) + 1`))
  (j1: Z)
  (Post7: j1 = `n2 - 1`)
  (Variant5: Z)
  (j2: Z)
  (old2: Z)
  (t3: (array `n2 + 1` Z))
  (Pre18: Variant5 = `j2 + 1`)
  (Pre17: `(-1) <= j2` /\ `j2 <= n2 - 1` /\
          ((k:Z)
           (`j2 < k` /\ `k <= n2` -> (min_suffix w1 w2 i3 k (access t3 k)))) /\
          ((k:Z)
           (`0 <= k` /\ `k <= j2` ->
            (min_suffix w1 w2 `i3 + 1` k (access t3 k)))) /\
          (min_suffix w1 w2 `i3 + 1` `j2 + 1` old2))
  (Test7: `j2 >= 0`)
  (result7: Z)
  (Post8: result7 = old2)
  (Pre10: `0 <= j2` /\ `j2 < n2 + 1`)
  (old3: Z)
  (Post9: old3 = (access t3 j2))
  (Test5: ~(access w1 i3) = (access w2 j2))
  (Pre13: `0 <= j2 + 1` /\ `j2 + 1 < n2 + 1`)
  (Pre14: `0 <= j2` /\ `j2 < n2 + 1`)
  (t4: (array `n2 + 1` Z))
  (Post10: t4 = (store t3 j2 `(Zmin (access t3 j2) (access t3 j2 + 1)) + 1`))
  ((j:Z)
   (j = `j2 - 1` -> `(-1) <= j` /\ `j <= n2 - 1` /\
    ((k:Z) (`j < k` /\ `k <= n2` -> (min_suffix w1 w2 i3 k (access t4 k)))) /\
    ((k:Z)
     (`0 <= k` /\ `k <= j` -> (min_suffix w1 w2 `i3 + 1` k (access t4 k)))) /\
    (min_suffix w1 w2 `i3 + 1` `j + 1` old3) /\ (Zwf `0` `j + 1` `j2 + 1`))).
Proof.
Intuition.
Unfold min_suffix.
Rewrite Post10; Clear Post10.
ArrayAccess j2 k Hj2k.
  (* j2=k *)
  Rewrite <- Hj2k.  
  Rewrite (suffix_is_cons ? w1 i3); [ Idtac | Omega' ].
  Rewrite (suffix_is_cons ? w2 j2); [ Idtac | Omega' ].
  Apply min_dist_diff. 
  Assumption.
  Rewrite <- (suffix_is_cons ? w1 i3); [ Idtac | Omega' ].
  Apply H11; Omega'.
  Rewrite <- (suffix_is_cons ? w2 j2); [ Idtac | Omega' ].
  Apply H13; Omega'.
  Omega'.
  (* j2<> k *)
  Apply H11; Omega'.
  Omega'.
  Omega'.
Rewrite Post10; Clear Post10.
Unfold min_suffix.
Rewrite store_def_2.
Apply H13; Omega'.
Omega'.
Omega'.
Omega'.
Replace `j+1` with j2; [ Idtac | Omega' ].
Rewrite Post9; Apply H13; Omega'.
Unfold Zwf; Omega'.
Save.

Lemma distance_po_17 : 
  (w1: (array n1 A))
  (w2: (array n2 A))
  (i0: Z)
  (Post1: i0 = `0`)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Post15: `0 <= i1` /\ `i1 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)) /\
           `i1 > n2`)
  (i2: Z)
  (Post4: i2 = `n1 - 1`)
  (Variant3: Z)
  (i3: Z)
  (t1: (array `n2 + 1` Z))
  (Pre20: Variant3 = `i3 + 1`)
  (Pre19: `(-1) <= i3` /\ `i3 <= n1 - 1` /\
          ((j:Z)
           (`0 <= j` /\ `j <= n2` ->
            (min_suffix w1 w2 `i3 + 1` j (access t1 j)))))
  (Test8: `i3 >= 0`)
  (Pre6: `0 <= n2` /\ `n2 < n2 + 1`)
  (old1: Z)
  (Post5: old1 = (access t1 n2))
  (Pre7: `0 <= n2` /\ `n2 < n2 + 1`)
  (t2: (array `n2 + 1` Z))
  (Post6: t2 = (store t1 n2 `(access t1 n2) + 1`))
  (j1: Z)
  (Post7: j1 = `n2 - 1`)
  (Variant5: Z)
  (j2: Z)
  (old2: Z)
  (t3: (array `n2 + 1` Z))
  (Pre18: Variant5 = `j2 + 1`)
  (Pre17: `(-1) <= j2` /\ `j2 <= n2 - 1` /\
          ((k:Z)
           (`j2 < k` /\ `k <= n2` -> (min_suffix w1 w2 i3 k (access t3 k)))) /\
          ((k:Z)
           (`0 <= k` /\ `k <= j2` ->
            (min_suffix w1 w2 `i3 + 1` k (access t3 k)))) /\
          (min_suffix w1 w2 `i3 + 1` `j2 + 1` old2))
  (Test7: `j2 >= 0`)
  (old3: Z)
  (t4: (array `n2 + 1` Z))
  (Post30: ((j:Z)
            (j = `j2 - 1` -> `(-1) <= j` /\ `j <= n2 - 1` /\
             ((k:Z)
              (`j < k` /\ `k <= n2` -> (min_suffix w1 w2 i3 k (access t4 k)))) /\
             ((k:Z)
              (`0 <= k` /\ `k <= j` ->
               (min_suffix w1 w2 `i3 + 1` k (access t4 k)))) /\
             (min_suffix w1 w2 `i3 + 1` `j + 1` old3) /\
             (Zwf `0` `j + 1` `j2 + 1`))))
  (j3: Z)
  (Post12: j3 = `j2 - 1`)
  `(-1) <= j3` /\ `j3 <= n2 - 1` /\
  ((k:Z) (`j3 < k` /\ `k <= n2` -> (min_suffix w1 w2 i3 k (access t4 k)))) /\
  ((k:Z)
   (`0 <= k` /\ `k <= j3` -> (min_suffix w1 w2 `i3 + 1` k (access t4 k)))) /\
  (min_suffix w1 w2 `i3 + 1` `j3 + 1` old3) /\ (Zwf `0` `j3 + 1` `j2 + 1`).
Proof. 
Intuition.
Save.

Lemma distance_po_18 : 
  (w1: (array n1 A))
  (w2: (array n2 A))
  (i0: Z)
  (Post1: i0 = `0`)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Post15: `0 <= i1` /\ `i1 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)) /\
           `i1 > n2`)
  (i2: Z)
  (Post4: i2 = `n1 - 1`)
  (Variant3: Z)
  (i3: Z)
  (t1: (array `n2 + 1` Z))
  (Pre20: Variant3 = `i3 + 1`)
  (Pre19: `(-1) <= i3` /\ `i3 <= n1 - 1` /\
          ((j:Z)
           (`0 <= j` /\ `j <= n2` ->
            (min_suffix w1 w2 `i3 + 1` j (access t1 j)))))
  (Test8: `i3 >= 0`)
  (Pre6: `0 <= n2` /\ `n2 < n2 + 1`)
  (old1: Z)
  (Post5: old1 = (access t1 n2))
  (Pre7: `0 <= n2` /\ `n2 < n2 + 1`)
  (t2: (array `n2 + 1` Z))
  (Post6: t2 = (store t1 n2 `(access t1 n2) + 1`))
  (j1: Z)
  (Post7: j1 = `n2 - 1`)
  (Variant5: Z)
  (j2: Z)
  (old2: Z)
  (t3: (array `n2 + 1` Z))
  (Pre18: Variant5 = `j2 + 1`)
  (Pre17: `(-1) <= j2` /\ `j2 <= n2 - 1` /\
          ((k:Z)
           (`j2 < k` /\ `k <= n2` -> (min_suffix w1 w2 i3 k (access t3 k)))) /\
          ((k:Z)
           (`0 <= k` /\ `k <= j2` ->
            (min_suffix w1 w2 `i3 + 1` k (access t3 k)))) /\
          (min_suffix w1 w2 `i3 + 1` `j2 + 1` old2))
  (Test7: `j2 >= 0`)
  (j3: Z)
  (old3: Z)
  (t4: (array `n2 + 1` Z))
  (Post29: `(-1) <= j3` /\ `j3 <= n2 - 1` /\
           ((k:Z)
            (`j3 < k` /\ `k <= n2` -> (min_suffix w1 w2 i3 k (access t4 k)))) /\
           ((k:Z)
            (`0 <= k` /\ `k <= j3` ->
             (min_suffix w1 w2 `i3 + 1` k (access t4 k)))) /\
           (min_suffix w1 w2 `i3 + 1` `j3 + 1` old3) /\
           (Zwf `0` `j3 + 1` `j2 + 1`))
  (Zwf `0` `j3 + 1` Variant5).
Proof.
Intuition.
Rewrite Pre18; Tauto.
Save.

Lemma distance_po_19 : 
  (w1: (array n1 A))
  (w2: (array n2 A))
  (i0: Z)
  (Post1: i0 = `0`)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Post15: `0 <= i1` /\ `i1 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)) /\
           `i1 > n2`)
  (i2: Z)
  (Post4: i2 = `n1 - 1`)
  (Variant3: Z)
  (i3: Z)
  (t1: (array `n2 + 1` Z))
  (Pre20: Variant3 = `i3 + 1`)
  (Pre19: `(-1) <= i3` /\ `i3 <= n1 - 1` /\
          ((j:Z)
           (`0 <= j` /\ `j <= n2` ->
            (min_suffix w1 w2 `i3 + 1` j (access t1 j)))))
  (Test8: `i3 >= 0`)
  (Pre6: `0 <= n2` /\ `n2 < n2 + 1`)
  (old1: Z)
  (Post5: old1 = (access t1 n2))
  (Pre7: `0 <= n2` /\ `n2 < n2 + 1`)
  (t2: (array `n2 + 1` Z))
  (Post6: t2 = (store t1 n2 `(access t1 n2) + 1`))
  (j1: Z)
  (Post7: j1 = `n2 - 1`)
  (Variant5: Z)
  (j2: Z)
  (old2: Z)
  (t3: (array `n2 + 1` Z))
  (Pre18: Variant5 = `j2 + 1`)
  (Pre17: `(-1) <= j2` /\ `j2 <= n2 - 1` /\
          ((k:Z)
           (`j2 < k` /\ `k <= n2` -> (min_suffix w1 w2 i3 k (access t3 k)))) /\
          ((k:Z)
           (`0 <= k` /\ `k <= j2` ->
            (min_suffix w1 w2 `i3 + 1` k (access t3 k)))) /\
          (min_suffix w1 w2 `i3 + 1` `j2 + 1` old2))
  (Test7: `j2 >= 0`)
  (j3: Z)
  (old3: Z)
  (t4: (array `n2 + 1` Z))
  (Post29: `(-1) <= j3` /\ `j3 <= n2 - 1` /\
           ((k:Z)
            (`j3 < k` /\ `k <= n2` -> (min_suffix w1 w2 i3 k (access t4 k)))) /\
           ((k:Z)
            (`0 <= k` /\ `k <= j3` ->
             (min_suffix w1 w2 `i3 + 1` k (access t4 k)))) /\
           (min_suffix w1 w2 `i3 + 1` `j3 + 1` old3) /\
           (Zwf `0` `j3 + 1` `j2 + 1`))
  `(-1) <= j3` /\ `j3 <= n2 - 1` /\
  ((k:Z) (`j3 < k` /\ `k <= n2` -> (min_suffix w1 w2 i3 k (access t4 k)))) /\
  ((k:Z)
   (`0 <= k` /\ `k <= j3` -> (min_suffix w1 w2 `i3 + 1` k (access t4 k)))) /\
  (min_suffix w1 w2 `i3 + 1` `j3 + 1` old3).
Proof.
Intuition.
Save.

Lemma distance_po_20 : 
  (w1: (array n1 A))
  (w2: (array n2 A))
  (i0: Z)
  (Post1: i0 = `0`)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Post15: `0 <= i1` /\ `i1 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)) /\
           `i1 > n2`)
  (i2: Z)
  (Post4: i2 = `n1 - 1`)
  (Variant3: Z)
  (i3: Z)
  (t1: (array `n2 + 1` Z))
  (Pre20: Variant3 = `i3 + 1`)
  (Pre19: `(-1) <= i3` /\ `i3 <= n1 - 1` /\
          ((j:Z)
           (`0 <= j` /\ `j <= n2` ->
            (min_suffix w1 w2 `i3 + 1` j (access t1 j)))))
  (Test8: `i3 >= 0`)
  (Pre6: `0 <= n2` /\ `n2 < n2 + 1`)
  (old1: Z)
  (Post5: old1 = (access t1 n2))
  (Pre7: `0 <= n2` /\ `n2 < n2 + 1`)
  (t2: (array `n2 + 1` Z))
  (Post6: t2 = (store t1 n2 `(access t1 n2) + 1`))
  (j1: Z)
  (Post7: j1 = `n2 - 1`)
  (Variant5: Z)
  (j2: Z)
  (old2: Z)
  (t3: (array `n2 + 1` Z))
  (Pre18: Variant5 = `j2 + 1`)
  (Pre17: `(-1) <= j2` /\ `j2 <= n2 - 1` /\
          ((k:Z)
           (`j2 < k` /\ `k <= n2` -> (min_suffix w1 w2 i3 k (access t3 k)))) /\
          ((k:Z)
           (`0 <= k` /\ `k <= j2` ->
            (min_suffix w1 w2 `i3 + 1` k (access t3 k)))) /\
          (min_suffix w1 w2 `i3 + 1` `j2 + 1` old2))
  (Test4: `j2 < 0`)
  `(-1) <= j2` /\ `j2 <= n2 - 1` /\
  ((k:Z) (`j2 < k` /\ `k <= n2` -> (min_suffix w1 w2 i3 k (access t3 k)))) /\
  ((k:Z)
   (`0 <= k` /\ `k <= j2` -> (min_suffix w1 w2 `i3 + 1` k (access t3 k)))) /\
  (min_suffix w1 w2 `i3 + 1` `j2 + 1` old2) /\ `j2 < 0`.
Proof.
Intuition.
Save.

Lemma distance_po_21 : 
  (w1: (array n1 A))
  (w2: (array n2 A))
  (i0: Z)
  (Post1: i0 = `0`)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Post15: `0 <= i1` /\ `i1 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)) /\
           `i1 > n2`)
  (i2: Z)
  (Post4: i2 = `n1 - 1`)
  (Variant3: Z)
  (i3: Z)
  (t1: (array `n2 + 1` Z))
  (Pre20: Variant3 = `i3 + 1`)
  (Pre19: `(-1) <= i3` /\ `i3 <= n1 - 1` /\
          ((j:Z)
           (`0 <= j` /\ `j <= n2` ->
            (min_suffix w1 w2 `i3 + 1` j (access t1 j)))))
  (Test8: `i3 >= 0`)
  (Pre6: `0 <= n2` /\ `n2 < n2 + 1`)
  (old1: Z)
  (Post5: old1 = (access t1 n2))
  (Pre7: `0 <= n2` /\ `n2 < n2 + 1`)
  (t2: (array `n2 + 1` Z))
  (Post6: t2 = (store t1 n2 `(access t1 n2) + 1`))
  (j1: Z)
  (Post7: j1 = `n2 - 1`)
  `(-1) <= j1` /\ `j1 <= n2 - 1` /\
  ((k:Z) (`j1 < k` /\ `k <= n2` -> (min_suffix w1 w2 i3 k (access t2 k)))) /\
  ((k:Z)
   (`0 <= k` /\ `k <= j1` -> (min_suffix w1 w2 `i3 + 1` k (access t2 k)))) /\
  (min_suffix w1 w2 `i3 + 1` `j1 + 1` old1).
Proof.
Intuition.
Rewrite Post6; Clear Post6.
Replace k with n2; [ Idtac | Omega' ].
Unfold min_suffix.
Rewrite (suffix_is_cons ? w1 i3).
Rewrite suffix_n_is_eps.
Rewrite store_def_1.
Apply min_dist_eps.
Rewrite <- suffix_n_is_eps with t:=w2.
Apply H8; Omega'.
Omega'.
Omega'.
Rewrite Post6.
Rewrite store_def_2.
Apply H8; Omega'.
Omega'.
Omega'.
Omega'.
Rewrite Post5.
Pattern 2 n2.
Replace `n2` with `(j1+1)`; [ Idtac | Omega' ].
Apply H8; Omega'.
Save.

Lemma distance_po_22 : 
  (w1: (array n1 A))
  (w2: (array n2 A))
  (i0: Z)
  (Post1: i0 = `0`)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Post15: `0 <= i1` /\ `i1 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)) /\
           `i1 > n2`)
  (i2: Z)
  (Post4: i2 = `n1 - 1`)
  (Variant3: Z)
  (i3: Z)
  (t1: (array `n2 + 1` Z))
  (Pre20: Variant3 = `i3 + 1`)
  (Pre19: `(-1) <= i3` /\ `i3 <= n1 - 1` /\
          ((j:Z)
           (`0 <= j` /\ `j <= n2` ->
            (min_suffix w1 w2 `i3 + 1` j (access t1 j)))))
  (Test8: `i3 >= 0`)
  (Pre6: `0 <= n2` /\ `n2 < n2 + 1`)
  (old1: Z)
  (Post5: old1 = (access t1 n2))
  (Pre7: `0 <= n2` /\ `n2 < n2 + 1`)
  (t2: (array `n2 + 1` Z))
  (Post6: t2 = (store t1 n2 `(access t1 n2) + 1`))
  (j1: Z)
  (Post7: j1 = `n2 - 1`)
  (j2: Z)
  (old2: Z)
  (t3: (array `n2 + 1` Z))
  (Post25: `(-1) <= j2` /\ `j2 <= n2 - 1` /\
           ((k:Z)
            (`j2 < k` /\ `k <= n2` -> (min_suffix w1 w2 i3 k (access t3 k)))) /\
           ((k:Z)
            (`0 <= k` /\ `k <= j2` ->
             (min_suffix w1 w2 `i3 + 1` k (access t3 k)))) /\
           (min_suffix w1 w2 `i3 + 1` `j2 + 1` old2) /\ `j2 < 0`)
  (i4: Z)
  (Post13: i4 = `i3 - 1`)
  `(-1) <= i4` /\ `i4 <= n1 - 1` /\
  ((j:Z)
   (`0 <= j` /\ `j <= n2` -> (min_suffix w1 w2 `i4 + 1` j (access t3 j)))) /\
  (Zwf `0` `i4 + 1` `i3 + 1`).
Proof.
Intuition.
Replace `i4+1` with i3; [ Idtac | Omega' ].
Apply H11; Omega'.
Unfold Zwf; Omega'.
Save.

Lemma distance_po_23 : 
  (w1: (array n1 A))
  (w2: (array n2 A))
  (i0: Z)
  (Post1: i0 = `0`)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Post15: `0 <= i1` /\ `i1 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)) /\
           `i1 > n2`)
  (i2: Z)
  (Post4: i2 = `n1 - 1`)
  (Variant3: Z)
  (i3: Z)
  (t1: (array `n2 + 1` Z))
  (Pre20: Variant3 = `i3 + 1`)
  (Pre19: `(-1) <= i3` /\ `i3 <= n1 - 1` /\
          ((j:Z)
           (`0 <= j` /\ `j <= n2` ->
            (min_suffix w1 w2 `i3 + 1` j (access t1 j)))))
  (Test8: `i3 >= 0`)
  (i4: Z)
  (t2: (array `n2 + 1` Z))
  (Post24: `(-1) <= i4` /\ `i4 <= n1 - 1` /\
           ((j:Z)
            (`0 <= j` /\ `j <= n2` ->
             (min_suffix w1 w2 `i4 + 1` j (access t2 j)))) /\
           (Zwf `0` `i4 + 1` `i3 + 1`))
  (Zwf `0` `i4 + 1` Variant3).
Proof.
Intuition.
Rewrite Pre20; Assumption.
Save.

Lemma distance_po_24 : 
  (w1: (array n1 A))
  (w2: (array n2 A))
  (i0: Z)
  (Post1: i0 = `0`)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Post15: `0 <= i1` /\ `i1 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)) /\
           `i1 > n2`)
  (i2: Z)
  (Post4: i2 = `n1 - 1`)
  (Variant3: Z)
  (i3: Z)
  (t1: (array `n2 + 1` Z))
  (Pre20: Variant3 = `i3 + 1`)
  (Pre19: `(-1) <= i3` /\ `i3 <= n1 - 1` /\
          ((j:Z)
           (`0 <= j` /\ `j <= n2` ->
            (min_suffix w1 w2 `i3 + 1` j (access t1 j)))))
  (Test8: `i3 >= 0`)
  (i4: Z)
  (t2: (array `n2 + 1` Z))
  (Post24: `(-1) <= i4` /\ `i4 <= n1 - 1` /\
           ((j:Z)
            (`0 <= j` /\ `j <= n2` ->
             (min_suffix w1 w2 `i4 + 1` j (access t2 j)))) /\
           (Zwf `0` `i4 + 1` `i3 + 1`))
  `(-1) <= i4` /\ `i4 <= n1 - 1` /\
  ((j:Z)
   (`0 <= j` /\ `j <= n2` -> (min_suffix w1 w2 `i4 + 1` j (access t2 j)))).
Proof. (* distance_po_24 *)
Intuition.
Save.

Lemma distance_po_25 : 
  (w1: (array n1 A))
  (w2: (array n2 A))
  (i0: Z)
  (Post1: i0 = `0`)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Post15: `0 <= i1` /\ `i1 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)) /\
           `i1 > n2`)
  (i2: Z)
  (Post4: i2 = `n1 - 1`)
  (Variant3: Z)
  (i3: Z)
  (t1: (array `n2 + 1` Z))
  (Pre20: Variant3 = `i3 + 1`)
  (Pre19: `(-1) <= i3` /\ `i3 <= n1 - 1` /\
          ((j:Z)
           (`0 <= j` /\ `j <= n2` ->
            (min_suffix w1 w2 `i3 + 1` j (access t1 j)))))
  (Test3: `i3 < 0`)
  `(-1) <= i3` /\ `i3 <= n1 - 1` /\
  ((j:Z)
   (`0 <= j` /\ `j <= n2` -> (min_suffix w1 w2 `i3 + 1` j (access t1 j)))) /\
  `i3 < 0`.
Proof. (* distance_po_25 *)
Intuition.
Save.

Lemma distance_po_26 : 
  (w1: (array n1 A))
  (w2: (array n2 A))
  (i0: Z)
  (Post1: i0 = `0`)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Post15: `0 <= i1` /\ `i1 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)) /\
           `i1 > n2`)
  (i2: Z)
  (Post4: i2 = `n1 - 1`)
  `(-1) <= i2` /\ `i2 <= n1 - 1` /\
  ((j:Z)
   (`0 <= j` /\ `j <= n2` -> (min_suffix w1 w2 `i2 + 1` j (access t0 j)))).
Proof. (* distance_po_26 *)
Intuition.
Omega'.
Replace `i2+1` with n1; [ Idtac | Omega' ].
Unfold min_suffix.
Rewrite suffix_n_is_eps.
Replace (access t0 j) with (Zlength (suffix w2 j)).
Exact (min_dist_eps_length (suffix w2 j)).
Rewrite H0.
Apply suffix_length; Omega'.
Omega'.
Save.

Lemma distance_po_27 : 
  (w1: (array n1 A))
  (w2: (array n2 A))
  (i0: Z)
  (Post1: i0 = `0`)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Post15: `0 <= i1` /\ `i1 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)) /\
           `i1 > n2`)
  (i2: Z)
  (Post4: i2 = `n1 - 1`)
  (i3: Z)
  (t1: (array `n2 + 1` Z))
  (Post20: `(-1) <= i3` /\ `i3 <= n1 - 1` /\
           ((j:Z)
            (`0 <= j` /\ `j <= n2` ->
             (min_suffix w1 w2 `i3 + 1` j (access t1 j)))) /\
           `i3 < 0`)
  `0 <= 0` /\ `0 < n2 + 1`.
Proof. (* distance_po_27 *)
Intuition.
Omega'.
Save.

Lemma distance_po_28 : 
  (w1: (array n1 A))
  (w2: (array n2 A))
  (i0: Z)
  (Post1: i0 = `0`)
  (i1: Z)
  (t0: (array `n2 + 1` Z))
  (Post15: `0 <= i1` /\ `i1 <= n2 + 1` /\
           ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)) /\
           `i1 > n2`)
  (i2: Z)
  (Post4: i2 = `n1 - 1`)
  (i3: Z)
  (t1: (array `n2 + 1` Z))
  (Post20: `(-1) <= i3` /\ `i3 <= n1 - 1` /\
           ((j:Z)
            (`0 <= j` /\ `j <= n2` ->
             (min_suffix w1 w2 `i3 + 1` j (access t1 j)))) /\
           `i3 < 0`)
  (Pre21: `0 <= 0` /\ `0 < n2 + 1`)
  (min_dist (word_of_array w1) (word_of_array w2) (access t1 `0`)).
Proof. (* distance_po_28 *)
Intuition.
Cut `i3+1=0`; [ Intro Hi3 | Omega' ].
Rewrite Hi3 in H4.
Unfold word_of_array.
Unfold min_suffix in H4.
Apply (H4 `0`); Omega'.
Save.








Definition distance := (* validation *)
  [i: Z; j: Z; old: Z; t: (array `n2 + 1` Z); w1: (array n1 A);
   w2: (array n2 A)]
    let (i0, result, Post1) =
      let (result, Post1) = (exist_1 [result: Z]result = `0` `0`
        (refl_equal ? `0`)) in
      (exist_2 [i1: Z][result0: unit]i1 = `0` result tt Post1) in
    let (i1, t0, result0, Post15) =
      (well_founded_induction Z (Zwf ZERO) (distance_po_1 i0 Post1)
        [Variant1: Z](i1: Z)(t0: (array `n2 + 1` Z))
        (_: Variant1 = `n2 + 1 - i1`)(_: `0 <= i1` /\ `i1 <= n2 + 1` /\
        ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)))
        (sig_3 Z (array `n2 + 1` Z) unit [i2:Z][t1:(array `n2 + 1` Z)]
         [result:unit](`0 <= i2` /\ `i2 <= n2 + 1` /\
         ((j:Z) (`0 <= j` /\ `j < i2` -> (access t1 j) = `n2 - j`)) /\
         `i2 > n2`))
        [Variant1: Z; wf1: (Variant2: Z)(Pre1: (Zwf `0` Variant2 Variant1))
         (i1: Z)(t0: (array `n2 + 1` Z))(_: Variant2 = `n2 + 1 - i1`)
         (_: `0 <= i1` /\ `i1 <= n2 + 1` /\
         ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`)))
         (sig_3 Z (array `n2 + 1` Z) unit [i2:Z][t1:(array `n2 + 1` Z)]
          [result:unit](`0 <= i2` /\ `i2 <= n2 + 1` /\
          ((j:Z) (`0 <= j` /\ `j < i2` -> (access t1 j) = `n2 - j`)) /\
          `i2 > n2`));
         i1: Z; t0: (array `n2 + 1` Z); Pre4: Variant1 = `n2 + 1 - i1`;
         Pre3: `0 <= i1` /\ `i1 <= n2 + 1` /\
         ((j:Z) (`0 <= j` /\ `j < i1` -> (access t0 j) = `n2 - j`))]
          let (result0, Bool1) =
            let (result2, Post16) = (Z_le_gt_bool i1 n2) in
            (exist_1 [result3: bool]
            (if result3 then `i1 <= n2` else `i1 > n2`) result2 Post16) in
          (Cases (btest
                  [result0:bool](if result0 then `i1 <= n2` else `i1 > n2`)
                  result0 Bool1) of
          | (left Test2) =>
              let (i2, t1, result1, Post18) =
                let (i2, t1, result1, Post19) =
                  let (t1, result1, Post2) =
                    let (result1, Post2) = (exist_1 [result1: Z]
                      (store t0 i1 result1) = (store t0 i1 `n2 - i1`) 
                      `n2 - i1` (refl_equal ? (store t0 i1 `n2 - i1`))) in
                    let Pre2 =
                      (distance_po_2 i0 Post1 Variant1 i1 t0 Pre4 Pre3 Test2
                      result1 Post2) in
                    (exist_2 [t2: (array `n2 + 1` Z)][result3: unit]
                    t2 = (store t0 i1 `n2 - i1`) (store t0 i1 result1) 
                    tt Post2) in
                  let (i2, result2, Post3) =
                    let (result2, Post3) = (exist_1 [result2: Z]
                      result2 = `i1 + 1` `i1 + 1` (refl_equal ? `i1 + 1`)) in
                    (exist_2 [i3: Z][result3: unit]i3 = `i1 + 1` result2 
                    tt Post3) in
                  (exist_3 [i3: Z][t2: (array `n2 + 1` Z)][result3: unit]
                  `0 <= i3` /\ `i3 <= n2 + 1` /\
                  ((j:Z) (`0 <= j` /\ `j < i3` -> (access t2 j) = `n2 - j`)) /\
                  (Zwf `0` `n2 + 1 - i3` `n2 + 1 - i1`) i2 t1 result2
                  (distance_po_3 i0 Post1 Variant1 i1 t0 Pre4 Pre3 Test2 t1
                  Post2 i2 Post3)) in
                ((wf1 `n2 + 1 - i2`)
                  (distance_po_4 i0 Post1 Variant1 i1 t0 Pre4 Pre3 Test2 i2
                  t1 Post19) i2 t1 (refl_equal ? `n2 + 1 - i2`)
                  (distance_po_5 i0 Post1 Variant1 i1 t0 Pre4 Pre3 Test2 i2
                  t1 Post19)) in
              (exist_3 [i3: Z][t2: (array `n2 + 1` Z)][result2: unit]
              `0 <= i3` /\ `i3 <= n2 + 1` /\
              ((j:Z) (`0 <= j` /\ `j < i3` -> (access t2 j) = `n2 - j`)) /\
              `i3 > n2` i2 t1 result1 Post18)
          | (right Test1) =>
              let (i2, t1, result1, Post17) = (exist_3 [i2: Z]
                [t1: (array `n2 + 1` Z)][result1: unit]`0 <= i2` /\
                `i2 <= n2 + 1` /\
                ((j:Z) (`0 <= j` /\ `j < i2` -> (access t1 j) = `n2 - j`)) /\
                `i2 > n2` i1 t0 tt
                (distance_po_6 i0 Post1 Variant1 i1 t0 Pre4 Pre3 Test1)) in
              (exist_3 [i3: Z][t2: (array `n2 + 1` Z)][result2: unit]
              `0 <= i3` /\ `i3 <= n2 + 1` /\
              ((j:Z) (`0 <= j` /\ `j < i3` -> (access t2 j) = `n2 - j`)) /\
              `i3 > n2` i2 t1 result1 Post17) end) `n2 + 1 - i0` i0 t
        (refl_equal ? `n2 + 1 - i0`) (distance_po_7 t i0 Post1)) in
    let (i2, result1, Post4) =
      let (result1, Post4) = (exist_1 [result1: Z]result1 = `n1 - 1` 
        `n1 - 1` (refl_equal ? `n1 - 1`)) in
      (exist_2 [i3: Z][result2: unit]i3 = `n1 - 1` result1 tt Post4) in
    let (i3, j0, old0, t1, result2, Post20) =
      (well_founded_induction Z (Zwf ZERO)
        (distance_po_8 i0 Post1 i1 t0 Post15 i2 Post4) [Variant3: Z](i3: Z)
        (j0: Z)(old0: Z)(t1: (array `n2 + 1` Z))(_: Variant3 = `i3 + 1`)
        (_: `(-1) <= i3` /\ `i3 <= n1 - 1` /\
        ((j:Z)
         (`0 <= j` /\ `j <= n2` ->
          (min_suffix w1 w2 `i3 + 1` j (access t1 j)))))
        (sig_5 Z Z Z (array `n2 + 1` Z) unit [i4:Z][j1:Z][old1:Z]
         [t2:(array `n2 + 1` Z)][result:unit](`(-1) <= i4` /\
         `i4 <= n1 - 1` /\
         ((j:Z)
          (`0 <= j` /\ `j <= n2` ->
           (min_suffix w1 w2 `i4 + 1` j (access t2 j)))) /\
         `i4 < 0`))
        [Variant3: Z; wf2: (Variant4: Z)(Pre5: (Zwf `0` Variant4 Variant3))
         (i3: Z)(j0: Z)(old0: Z)(t1: (array `n2 + 1` Z))
         (_: Variant4 = `i3 + 1`)(_: `(-1) <= i3` /\ `i3 <= n1 - 1` /\
         ((j:Z)
          (`0 <= j` /\ `j <= n2` ->
           (min_suffix w1 w2 `i3 + 1` j (access t1 j)))))
         (sig_5 Z Z Z (array `n2 + 1` Z) unit [i4:Z][j1:Z][old1:Z]
          [t2:(array `n2 + 1` Z)][result:unit](`(-1) <= i4` /\
          `i4 <= n1 - 1` /\
          ((j:Z)
           (`0 <= j` /\ `j <= n2` ->
            (min_suffix w1 w2 `i4 + 1` j (access t2 j)))) /\
          `i4 < 0`));
         i3: Z; j0: Z; old0: Z; t1: (array `n2 + 1` Z);
         Pre20: Variant3 = `i3 + 1`; Pre19: `(-1) <= i3` /\ `i3 <= n1 - 1` /\
         ((j:Z)
          (`0 <= j` /\ `j <= n2` ->
           (min_suffix w1 w2 `i3 + 1` j (access t1 j))))]
          let (result2, Bool2) =
            let (result4, Post21) = (Z_ge_lt_bool i3 `0`) in
            (exist_1 [result5: bool]
            (if result5 then `i3 >= 0` else `i3 < 0`) result4 Post21) in
          (Cases (btest
                  [result2:bool](if result2 then `i3 >= 0` else `i3 < 0`)
                  result2 Bool2) of
          | (left Test8) =>
              let (i4, j1, old1, t2, result3, Post23) =
                let (i4, j1, old1, t2, result3, Post24) =
                  let Pre6 =
                    (distance_po_9 w1 w2 i0 Post1 i1 t0 Post15 i2 Post4
                    Variant3 i3 t1 Pre20 Pre19 Test8) in
                  let (old1, result3, Post5) =
                    let (result3, Post5) = (exist_1 [result3: Z]
                      result3 = (access t1 n2) (access t1 n2)
                      (refl_equal ? (access t1 n2))) in
                    (exist_2 [old2: Z][result4: unit]
                    old2 = (access t1 n2) result3 tt Post5) in
                  let Pre7 = Pre6 in
                  let (t2, result4, Post6) =
                    let (result4, Post6) = (exist_1 [result4: Z]
                      (store t1 n2 result4) =
                      (store t1 n2 `(access t1 n2) + 1`) `(access t1 n2) + 1`
                      (refl_equal ? (store t1 n2 `(access t1 n2) + 1`))) in
                    let Pre8 = Pre7 in
                    (exist_2 [t3: (array `n2 + 1` Z)][result6: unit]
                    t3 = (store t1 n2 `(access t1 n2) + 1`) (store t1 n2
                                                             result4)
                    tt Post6) in
                  let (j1, result5, Post7) =
                    let (result5, Post7) = (exist_1 [result5: Z]
                      result5 = `n2 - 1` `n2 - 1` (refl_equal ? `n2 - 1`)) in
                    (exist_2 [j2: Z][result6: unit]j2 = `n2 - 1` result5 
                    tt Post7) in
                  let (j2, old2, t3, result6, Post25) =
                    (well_founded_induction Z (Zwf ZERO)
                      (distance_po_10 w1 w2 i0 Post1 i1 t0 Post15 i2 Post4
                      Variant3 i3 t1 Pre20 Pre19 Test8 Pre6 old1 Post5 Pre7
                      t2 Post6 j1 Post7) [Variant5: Z](j2: Z)(old2: Z)
                      (t3: (array `n2 + 1` Z))(_: Variant5 = `j2 + 1`)
                      (_: `(-1) <= j2` /\ `j2 <= n2 - 1` /\
                      ((k:Z)
                       (`j2 < k` /\ `k <= n2` ->
                        (min_suffix w1 w2 i3 k (access t3 k)))) /\
                      ((k:Z)
                       (`0 <= k` /\ `k <= j2` ->
                        (min_suffix w1 w2 `i3 + 1` k (access t3 k)))) /\
                      (min_suffix w1 w2 `i3 + 1` `j2 + 1` old2))
                      (sig_4 Z Z (array `n2 + 1` Z) unit [j3:Z][old3:Z]
                       [t4:(array `n2 + 1` Z)][result:unit](`(-1) <= j3` /\
                       `j3 <= n2 - 1` /\
                       ((k:Z)
                        (`j3 < k` /\ `k <= n2` ->
                         (min_suffix w1 w2 i3 k (access t4 k)))) /\
                       ((k:Z)
                        (`0 <= k` /\ `k <= j3` ->
                         (min_suffix w1 w2 `i3 + 1` k (access t4 k)))) /\
                       (min_suffix w1 w2 `i3 + 1` `j3 + 1` old3) /\ `
                       j3 < 0`))
                      [Variant5: Z; wf3: (Variant6: Z)
                       (Pre9: (Zwf `0` Variant6 Variant5))(j2: Z)(old2: Z)
                       (t3: (array `n2 + 1` Z))(_: Variant6 = `j2 + 1`)
                       (_: `(-1) <= j2` /\ `j2 <= n2 - 1` /\
                       ((k:Z)
                        (`j2 < k` /\ `k <= n2` ->
                         (min_suffix w1 w2 i3 k (access t3 k)))) /\
                       ((k:Z)
                        (`0 <= k` /\ `k <= j2` ->
                         (min_suffix w1 w2 `i3 + 1` k (access t3 k)))) /\
                       (min_suffix w1 w2 `i3 + 1` `j2 + 1` old2))
                       (sig_4 Z Z (array `n2 + 1` Z) unit [j3:Z][old3:Z]
                        [t4:(array `n2 + 1` Z)][result:unit](`(-1) <= j3` /\
                        `j3 <= n2 - 1` /\
                        ((k:Z)
                         (`j3 < k` /\ `k <= n2` ->
                          (min_suffix w1 w2 i3 k (access t4 k)))) /\
                        ((k:Z)
                         (`0 <= k` /\ `k <= j3` ->
                          (min_suffix w1 w2 `i3 + 1` k (access t4 k)))) /\
                        (min_suffix w1 w2 `i3 + 1` `j3 + 1` old3) /\
                        `j3 < 0`));
                       j2: Z; old2: Z; t3: (array `n2 + 1` Z);
                       Pre18: Variant5 = `j2 + 1`; Pre17: `(-1) <= j2` /\
                       `j2 <= n2 - 1` /\
                       ((k:Z)
                        (`j2 < k` /\ `k <= n2` ->
                         (min_suffix w1 w2 i3 k (access t3 k)))) /\
                       ((k:Z)
                        (`0 <= k` /\ `k <= j2` ->
                         (min_suffix w1 w2 `i3 + 1` k (access t3 k)))) /\
                       (min_suffix w1 w2 `i3 + 1` `j2 + 1` old2)]
                        let (result6, Bool3) =
                          let (result8, Post26) = (Z_ge_lt_bool j2 `0`) in
                          (exist_1 [result9: bool]
                          (if result9 then `j2 >= 0` else `j2 < 0`) result8
                          Post26) in
                        (Cases (btest
                                [result6:bool](if result6 then `j2 >= 0`
                                               else `j2 < 0`)
                                result6 Bool3) of
                        | (left Test7) =>
                            let (j3, old3, t4, result7, Post28) =
                              let (j3, old3, t4, result7, Post29) =
                                let (old3, t4, result7, Post30) =
                                  let (result7, Post8) =
                                    (exist_1 [result7: Z]result7 = old2 
                                    old2 (refl_equal ? old2)) in
                                  let (old3, t4, result8, Post31) =
                                    let Pre10 =
                                      (distance_po_11 w1 w2 i0 Post1 i1 t0
                                      Post15 i2 Post4 Variant3 i3 t1 Pre20
                                      Pre19 Test8 Pre6 old1 Post5 Pre7 t2
                                      Post6 j1 Post7 Variant5 j2 old2 t3
                                      Pre18 Pre17 Test7 result7 Post8) in
                                    let (old3, result8, Post9) =
                                      let (result8, Post9) =
                                        (exist_1 [result8: Z]
                                        result8 = (access t3 j2) (access t3
                                                                  j2)
                                        (refl_equal ? (access t3 j2))) in
                                      (exist_2 [old4: Z][result9: unit]
                                      old4 = (access t3 j2) result8 tt Post9) in
                                    let (t4, result9, Post32) =
                                      let (result9, Bool4) =
                                        let Pre11 =
                                          (distance_po_12 w1 w2 i0 Post1 i1
                                          t0 Post15 i2 Post4 Variant3 i3 t1
                                          Pre20 Pre19 Test8 Pre6 old1 Post5
                                          Pre7 t2 Post6 j1 Post7 Variant5 j2
                                          old2 t3 Pre18 Pre17 Test7 result7
                                          Post8 Pre10 old3 Post9) in
                                        let result10 =
                                          let Pre12 =
                                            (distance_po_13 w1 w2 i0 Post1 i1
                                            t0 Post15 i2 Post4 Variant3 i3 t1
                                            Pre20 Pre19 Test8 Pre6 old1 Post5
                                            Pre7 t2 Post6 j1 Post7 Variant5
                                            j2 old2 t3 Pre18 Pre17 Test7
                                            result7 Post8 Pre10 old3 Post9
                                            Pre11) in
                                          (test_char (access w1 i3)) in
                                        let (result11, Post33) =
                                          (result10 (access w2 j2)) in
                                        (exist_1 [result12: bool]
                                        (if result12
                                         then (access w1 i3) = (access w2 j2)
                                         else ~(access w1 i3) =
                                               (access w2 j2)) result11
                                        Post33) in
                                      (Cases (btest
                                              [result9:bool](if result9
                                                             then (access w1
                                                                   i3) =
                                                                  (access w2
                                                                   j2)
                                                             else ~(access w1
                                                                    i3) =
                                                                   (access w2
                                                                    j2))
                                              result9 Bool4) of
                                      | (left Test6) =>
                                          let (t4, result10, Post11) =
                                            let (result10, Post11) =
                                              (exist_1 [result10: Z]
                                              (store t3 j2 result10) =
                                              (store t3 j2 result7) result7
                                              (refl_equal ? (store t3 j2
                                                             result7))) in
                                            let Pre16 = Pre10 in
                                            (exist_2 [t5: (array `n2 + 1` Z)]
                                            [result12: unit]
                                            t5 = (store t3 j2 result7) 
                                            (store t3 j2 result10) tt Post11) in
                                          (exist_2 [t5: (array `n2 + 1` Z)]
                                          [result11: unit]
                                          ((j:Z)
                                           (j = `j2 - 1` -> `(-1) <= j` /\
                                            `j <= n2 - 1` /\
                                            ((k:Z)
                                             (`j < k` /\ `k <= n2` ->
                                              (min_suffix w1 w2 i3 k
                                               (access t5 k)))) /\
                                            ((k:Z)
                                             (`0 <= k` /\ `k <= j` ->
                                              (min_suffix w1 w2 `i3 + 1` k
                                               (access t5 k)))) /\
                                            (min_suffix w1 w2 `i3 + 1`
                                             `j + 1` old3) /\
                                            (Zwf `0` `j + 1` `j2 + 1`))) 
                                          t4 result10
                                          (distance_po_14 w1 w2 i0 Post1 i1
                                          t0 Post15 i2 Post4 Variant3 i3 t1
                                          Pre20 Pre19 Test8 Pre6 old1 Post5
                                          Pre7 t2 Post6 j1 Post7 Variant5 j2
                                          old2 t3 Pre18 Pre17 Test7 result7
                                          Post8 Pre10 old3 Post9 Test6 t4
                                          Post11))
                                      | (right Test5) =>
                                          let Pre13 =
                                            (distance_po_15 w1 w2 i0 Post1 i1
                                            t0 Post15 i2 Post4 Variant3 i3 t1
                                            Pre20 Pre19 Test8 Pre6 old1 Post5
                                            Pre7 t2 Post6 j1 Post7 Variant5
                                            j2 old2 t3 Pre18 Pre17 Test7
                                            result7 Post8 Pre10 old3 Post9
                                            Test5) in
                                          let Pre14 = Pre10 in
                                          let (t4, result10, Post10) =
                                            let (result10, Post10) =
                                              (exist_1 [result10: Z]
                                              (store t3 j2 result10) =
                                              (store t3 j2
                                               `(Zmin (access t3 j2)
                                                 (access t3 j2 + 1)) +
                                                1`) `(Zmin (access t3 j2)
                                                      (access t3 j2 + 1)) +
                                                     1`
                                              (refl_equal ? (store t3 j2
                                                             `(Zmin (
                                                               access t3 j2)
                                                               (access t3
                                                                j2 + 1)) +
                                                              1`))) in
                                            let Pre15 = Pre14 in
                                            (exist_2 [t5: (array `n2 + 1` Z)]
                                            [result12: unit]
                                            t5 =
                                            (store t3 j2
                                             `(Zmin (access t3 j2)
                                               (access t3 j2 + 1)) +
                                              1`) (store t3 j2 result10)
                                            tt Post10) in
                                          (exist_2 [t5: (array `n2 + 1` Z)]
                                          [result11: unit]
                                          ((j:Z)
                                           (j = `j2 - 1` -> `(-1) <= j` /\
                                            `j <= n2 - 1` /\
                                            ((k:Z)
                                             (`j < k` /\ `k <= n2` ->
                                              (min_suffix w1 w2 i3 k
                                               (access t5 k)))) /\
                                            ((k:Z)
                                             (`0 <= k` /\ `k <= j` ->
                                              (min_suffix w1 w2 `i3 + 1` k
                                               (access t5 k)))) /\
                                            (min_suffix w1 w2 `i3 + 1`
                                             `j + 1` old3) /\
                                            (Zwf `0` `j + 1` `j2 + 1`))) 
                                          t4 result10
                                          (distance_po_16 w1 w2 i0 Post1 i1
                                          t0 Post15 i2 Post4 Variant3 i3 t1
                                          Pre20 Pre19 Test8 Pre6 old1 Post5
                                          Pre7 t2 Post6 j1 Post7 Variant5 j2
                                          old2 t3 Pre18 Pre17 Test7 result7
                                          Post8 Pre10 old3 Post9 Test5 Pre13
                                          Pre14 t4 Post10)) end) in
                                    (exist_3 [old4: Z]
                                    [t5: (array `n2 + 1` Z)][result10: unit]
                                    ((j:Z)
                                     (j = `j2 - 1` -> `(-1) <= j` /\
                                      `j <= n2 - 1` /\
                                      ((k:Z)
                                       (`j < k` /\ `k <= n2` ->
                                        (min_suffix w1 w2 i3 k (access t5 k)))) /\
                                      ((k:Z)
                                       (`0 <= k` /\ `k <= j` ->
                                        (min_suffix w1 w2 `i3 + 1` k
                                         (access t5 k)))) /\
                                      (min_suffix w1 w2 `i3 + 1` `j + 1` old4) /\
                                      (Zwf `0` `j + 1` `j2 + 1`))) old3
                                    t4 result9 Post32) in
                                  (exist_3 [old4: Z][t5: (array `n2 + 1` Z)]
                                  [result9: unit]
                                  ((j:Z)
                                   (j = `j2 - 1` -> `(-1) <= j` /\
                                    `j <= n2 - 1` /\
                                    ((k:Z)
                                     (`j < k` /\ `k <= n2` ->
                                      (min_suffix w1 w2 i3 k (access t5 k)))) /\
                                    ((k:Z)
                                     (`0 <= k` /\ `k <= j` ->
                                      (min_suffix w1 w2 `i3 + 1` k
                                       (access t5 k)))) /\
                                    (min_suffix w1 w2 `i3 + 1` `j + 1` old4) /\
                                    (Zwf `0` `j + 1` `j2 + 1`))) old3
                                  t4 result8 Post31) in
                                let (j3, result8, Post12) =
                                  let (result8, Post12) =
                                    (exist_1 [result8: Z]
                                    result8 = `j2 - 1` `j2 - 1`
                                    (refl_equal ? `j2 - 1`)) in
                                  (exist_2 [j4: Z][result9: unit]
                                  j4 = `j2 - 1` result8 tt Post12) in
                                (exist_4 [j4: Z][old4: Z]
                                [t5: (array `n2 + 1` Z)][result9: unit]
                                `(-1) <= j4` /\ `j4 <= n2 - 1` /\
                                ((k:Z)
                                 (`j4 < k` /\ `k <= n2` ->
                                  (min_suffix w1 w2 i3 k (access t5 k)))) /\
                                ((k:Z)
                                 (`0 <= k` /\ `k <= j4` ->
                                  (min_suffix w1 w2 `i3 + 1` k (access t5 k)))) /\
                                (min_suffix w1 w2 `i3 + 1` `j4 + 1` old4) /\
                                (Zwf `0` `j4 + 1` `j2 + 1`) j3 old3 t4
                                result8
                                (distance_po_17 w1 w2 i0 Post1 i1 t0 Post15
                                i2 Post4 Variant3 i3 t1 Pre20 Pre19 Test8
                                Pre6 old1 Post5 Pre7 t2 Post6 j1 Post7
                                Variant5 j2 old2 t3 Pre18 Pre17 Test7 old3 t4
                                Post30 j3 Post12)) in
                              ((wf3 `j3 + 1`)
                                (distance_po_18 w1 w2 i0 Post1 i1 t0 Post15
                                i2 Post4 Variant3 i3 t1 Pre20 Pre19 Test8
                                Pre6 old1 Post5 Pre7 t2 Post6 j1 Post7
                                Variant5 j2 old2 t3 Pre18 Pre17 Test7 j3 old3
                                t4 Post29) j3 old3 t4 (refl_equal ? `j3 + 1`)
                                (distance_po_19 w1 w2 i0 Post1 i1 t0 Post15
                                i2 Post4 Variant3 i3 t1 Pre20 Pre19 Test8
                                Pre6 old1 Post5 Pre7 t2 Post6 j1 Post7
                                Variant5 j2 old2 t3 Pre18 Pre17 Test7 j3 old3
                                t4 Post29)) in
                            (exist_4 [j4: Z][old4: Z][t5: (array `n2 + 1` Z)]
                            [result8: unit]`(-1) <= j4` /\ `j4 <= n2 - 1` /\
                            ((k:Z)
                             (`j4 < k` /\ `k <= n2` ->
                              (min_suffix w1 w2 i3 k (access t5 k)))) /\
                            ((k:Z)
                             (`0 <= k` /\ `k <= j4` ->
                              (min_suffix w1 w2 `i3 + 1` k (access t5 k)))) /\
                            (min_suffix w1 w2 `i3 + 1` `j4 + 1` old4) /\
                            `j4 < 0` j3 old3 t4 result7 Post28)
                        | (right Test4) =>
                            let (j3, old3, t4, result7, Post27) =
                              (exist_4 [j3: Z][old3: Z]
                              [t4: (array `n2 + 1` Z)][result7: unit]
                              `(-1) <= j3` /\ `j3 <= n2 - 1` /\
                              ((k:Z)
                               (`j3 < k` /\ `k <= n2` ->
                                (min_suffix w1 w2 i3 k (access t4 k)))) /\
                              ((k:Z)
                               (`0 <= k` /\ `k <= j3` ->
                                (min_suffix w1 w2 `i3 + 1` k (access t4 k)))) /\
                              (min_suffix w1 w2 `i3 + 1` `j3 + 1` old3) /\
                              `j3 < 0` j2 old2 t3 tt
                              (distance_po_20 w1 w2 i0 Post1 i1 t0 Post15 i2
                              Post4 Variant3 i3 t1 Pre20 Pre19 Test8 Pre6
                              old1 Post5 Pre7 t2 Post6 j1 Post7 Variant5 j2
                              old2 t3 Pre18 Pre17 Test4)) in
                            (exist_4 [j4: Z][old4: Z][t5: (array `n2 + 1` Z)]
                            [result8: unit]`(-1) <= j4` /\ `j4 <= n2 - 1` /\
                            ((k:Z)
                             (`j4 < k` /\ `k <= n2` ->
                              (min_suffix w1 w2 i3 k (access t5 k)))) /\
                            ((k:Z)
                             (`0 <= k` /\ `k <= j4` ->
                              (min_suffix w1 w2 `i3 + 1` k (access t5 k)))) /\
                            (min_suffix w1 w2 `i3 + 1` `j4 + 1` old4) /\
                            `j4 < 0` j3 old3 t4 result7 Post27) end) 
                      `j1 + 1` j1 old1 t2 (refl_equal ? `j1 + 1`)
                      (distance_po_21 w1 w2 i0 Post1 i1 t0 Post15 i2 Post4
                      Variant3 i3 t1 Pre20 Pre19 Test8 Pre6 old1 Post5 Pre7
                      t2 Post6 j1 Post7)) in
                  let (i4, result7, Post13) =
                    let (result7, Post13) = (exist_1 [result7: Z]
                      result7 = `i3 - 1` `i3 - 1` (refl_equal ? `i3 - 1`)) in
                    (exist_2 [i5: Z][result8: unit]i5 = `i3 - 1` result7 
                    tt Post13) in
                  (exist_5 [i5: Z][j3: Z][old3: Z][t4: (array `n2 + 1` Z)]
                  [result8: unit]`(-1) <= i5` /\ `i5 <= n1 - 1` /\
                  ((j:Z)
                   (`0 <= j` /\ `j <= n2` ->
                    (min_suffix w1 w2 `i5 + 1` j (access t4 j)))) /\
                  (Zwf `0` `i5 + 1` `i3 + 1`) i4 j2 old2 t3 result7
                  (distance_po_22 w1 w2 i0 Post1 i1 t0 Post15 i2 Post4
                  Variant3 i3 t1 Pre20 Pre19 Test8 Pre6 old1 Post5 Pre7 t2
                  Post6 j1 Post7 j2 old2 t3 Post25 i4 Post13)) in
                ((wf2 `i4 + 1`)
                  (distance_po_23 w1 w2 i0 Post1 i1 t0 Post15 i2 Post4
                  Variant3 i3 t1 Pre20 Pre19 Test8 i4 t2 Post24) i4 j1 
                  old1 t2 (refl_equal ? `i4 + 1`)
                  (distance_po_24 w1 w2 i0 Post1 i1 t0 Post15 i2 Post4
                  Variant3 i3 t1 Pre20 Pre19 Test8 i4 t2 Post24)) in
              (exist_5 [i5: Z][j2: Z][old2: Z][t3: (array `n2 + 1` Z)]
              [result4: unit]`(-1) <= i5` /\ `i5 <= n1 - 1` /\
              ((j:Z)
               (`0 <= j` /\ `j <= n2` ->
                (min_suffix w1 w2 `i5 + 1` j (access t3 j)))) /\
              `i5 < 0` i4 j1 old1 t2 result3 Post23)
          | (right Test3) =>
              let (i4, j1, old1, t2, result3, Post22) = (exist_5 [i4: Z]
                [j1: Z][old1: Z][t2: (array `n2 + 1` Z)][result3: unit]
                `(-1) <= i4` /\ `i4 <= n1 - 1` /\
                ((j:Z)
                 (`0 <= j` /\ `j <= n2` ->
                  (min_suffix w1 w2 `i4 + 1` j (access t2 j)))) /\
                `i4 < 0` i3 j0 old0 t1 tt
                (distance_po_25 w1 w2 i0 Post1 i1 t0 Post15 i2 Post4 Variant3
                i3 t1 Pre20 Pre19 Test3)) in
              (exist_5 [i5: Z][j2: Z][old2: Z][t3: (array `n2 + 1` Z)]
              [result4: unit]`(-1) <= i5` /\ `i5 <= n1 - 1` /\
              ((j:Z)
               (`0 <= j` /\ `j <= n2` ->
                (min_suffix w1 w2 `i5 + 1` j (access t3 j)))) /\
              `i5 < 0` i4 j1 old1 t2 result3 Post22) end) `i2 + 1` i2 
        j old t0 (refl_equal ? `i2 + 1`)
        (distance_po_26 w1 w2 i0 Post1 i1 t0 Post15 i2 Post4)) in
    let Pre21 =
      (distance_po_27 w1 w2 i0 Post1 i1 t0 Post15 i2 Post4 i3 t1 Post20) in
    let (result3, Post34) = (exist_1 [result3: Z]
      (min_dist (word_of_array w1) (word_of_array w2) result3) (access t1 `0`)
      (distance_po_28 w1 w2 i0 Post1 i1 t0 Post15 i2 Post4 i3 t1 Post20
      Pre21)) in
    (exist_5 [i4: Z][j1: Z][old1: Z][t2: (array `n2 + 1` Z)][result4: Z]
    (min_dist (word_of_array w1) (word_of_array w2) result4) i3 j0 old0 
    t1 result3 Post34).

