...
Theorem num_of_pos_non_negative : forall (int_P_int_M_t_3_8_at_L:(memory
  int_P int32)), forall (i_3:Z), forall (j_3:Z), forall (t_3:(pointer
  int_P)), (0%Z <= (num_of_pos i_3 j_3 t_3 int_P_int_M_t_3_8_at_L))%Z.
(* YOU MAY EDIT THE PROOF BELOW *)
intros tMem i j t.
assert (h: i>=j \/ i <j) by omega.
destruct h.
(* case i >= j *)
rewrite num_of_pos_empty; auto with zarith.
(* case i < j *)
apply Zlt_lower_bound_ind with 
 (P := fun j => 0 <= num_of_pos i j t tMem)
 (z:=i); auto with zarith.
intros j0 Hind Hj0.
assert (h: i=j0 \/ i < j0) by omega.
destruct h.
rewrite num_of_pos_empty; auto with zarith.
destruct (Z_lt_dec 0 (integer_of_int32 (select tMem (shift t (j0 - 1))))).
rewrite num_of_pos_true_case; intuition.
rewrite num_of_pos_false_case; intuition.
Qed.
(* DO NOT EDIT BELOW *)
