...
system, when there are loops like the \verb|while| of Schorr-Waite, it
is mandatory (except in simple cases) to manually add a loop
invariant. The design of a suitable loop invariant is the most
difficult part of this verification process. Fortunately for us, suitable loop
invariants were proposed by previous works on Schorr-Waite algorithm.
However, some difficulties remain. The main one is that we need to
express this loop invariant in the \caduceus{} syntax, which is
limited to a first-order logic. Hence, the higher-order specifications
of~\cite{mehta03cade} are not allowed by \caduceus.
...
//@ predicate in_list(node p,plist l)\end{slshape}
\end{alltt}
...
\item[I4a] defines the backtracking stack as on
  ...
  before. The precise definition of it will be given in Section~\ref{sec:coq}.
...
Simplify. These obligations are apparently too difficult, so we switched
to an interactive prover: the Coq proof assistant.
...
\verb|plist| used to model the stack: we simply use the finite list
...
\verb|clr_list|(a,c,l,r,p,cons(p,s)) \verb|:-| \verb|valid|(a,p),\\
  \verb|    clr_list|(a,next(c,l,r,p),s).
\end{array}
\]
where 
\[
\begin{array}{l}
next(c,l,r,p) = \\
\quad \verb|if|~select(c,p) =
0~\verb|then|~select(l,p)~\verb|else|~select(r,p)
\end{array}
\]
It may be surprising at first that the \verb|clr_list| predicate
defined here has six arguments instead of two, as declared in
Section~\ref{sec:invariant}. This is the effect of the \texttt{reads}
clause, explained first in Section~\ref{sec:spec}. As already said,
...
proceed in a slightly different way than Mehta and
Nipkow~\cite{mehta03cade}, who use a set-theory like axiomatization:
we use inductively defined predicates, which are more suitable for
Coq. 

We first introduce a more general predicate \verb|path| which also
makes precise the path between nodes. It is defined by three clauses:
\[
\begin{array}{l}
...
\qquad \exists s,~\verb|path|(a,l,r, p_1,
p_2, s) \land \\
...
\[
\begin{array}{l}
\forall a, p_1, p_2, l, r, s,~\verb|path|(a,l,r,p_1,p_2,s) \rightarrow
 \\\qquad
...
\verb|   | \forall p, p \in s \leftrightarrow \verb|reachable|(a,l,r,root,p) 
\end{array}
\]
...
the additional proof of termination).
