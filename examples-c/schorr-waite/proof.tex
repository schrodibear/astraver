...
/*@ predicate pair_in_list(node p1,
                    node p2,plist l) */\end{slshape}
\end{alltt}
...
\begin{alltt}\begin{slshape}
/*@ predicate unmarked_reachable (node p1,
                                  node p2) 
...
/*@ predicate reachable_elements(node root,
                                 plist s) 
            reads root->l,root->r */ 
/*@ requires 
       \bs{}exists plist s; 
         reachable_elements(root,s) && 
     ... */\end{slshape}
void schorr_waite(node root) \{
  node t = root; node p = NULL;\begin{slshape}
  /*@ invariant ...
