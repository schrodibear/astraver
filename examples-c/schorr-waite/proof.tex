...
\put(20,90){\rnode{root}{\makebox(0,0)[rb]{root}}}
...
\put(0,50){\rnode{p}{\makebox(0,0)[rb]{p}}}
...
element to a list, a predicate \verb|in_list| which tests if a pointer
...
invariant has eight assertions:
\begin {enumerate}
\item[I1] means that all nodes which were initially reachable from
  root, are reachable from $t$ or $p$
\item[I2] means that conversely, all (non-null) nodes reachable by $t$
  or $p$ were initially reachable from \verb|root|. This is true only
  for non-null node, because \verb|null| may not be reachable from
  \verb|root| is the graph is cyclic.
\item[I3] means that for initially unreachable nodes, the marks
  remain unchanged.
...
\item[I4c] is the trickiest annotation, means that
...
\item[I4e] is a tricky one also, that is essential for being able to
  `recover' the correct children of nodes of the stack, as it can be
  seen again on Figure~\ref{fig:stack}: if $p_1$ is in
  the stack, followed by $p_2$ (or if $p_1$ is $t$ and $p2$ is the head of the
...
can be defined inductively with the Prolog-like clauses (this is only
an approximation of the real Coq code, to ease reading, the real code
can be found in the \caduceus{} distribution):
\[
\begin{array}{l}
...
\verb|path| which also make precise the path between nodes. It is
...
p_2, s) \land \forall p, in(p,s) \rightarrow select(m,p) = 0 \\
\end{array} 
\]
Notice that it is now easy to establish the validity of the axiom
\verb|reachable_refl|.

A few lemmas have been proved before proving the obligations, the most
important one was to show that when $p_2$ is reachable from $p_1$,
there is a path which have no cycle:
\[
\begin{array}{l}
...
With those definitions and lemmas, we have been able to manually
complete all the proof obligations in Coq. We will give a few figures
...
Proving termination of the algorithm of course amounts to prove
...
to some well-founded ordering. It is quite easy to find such a
measure: there is a triple of natural numbers which decrease
...
\end{enumerate}
There is a minor problem: in our modeling, there is no assumption on
the finiteness of the memory, so indeed nothing tells that the set of
reachable nodes is finite. So indeed, this is an extra assumption we have
to insert in the pre-condition. Finally, the annotated source code is
completed as given Figure~\ref{fig:term}, where in Coq we define
\[
\begin{array}{l}
\verb|reachable_elements|(a,l,r, root, s) = %\\\qquad\qquad\qquad 
\forall p, in(p,s) \leftrightarrow reachable(a,l,r,root,p) 
\end{array}
\]
The \verb|weight| function and the \verb|order_mark_m_and_c_and_stack|
are also defined in Coq according the informal definition of the
measure above. 
...
With the completed annotated code, \caduceus{} generated two
...
far more difficult than the others.
...
lines, and 2411 lines of tactics were inserted manually for proving
them. Approximately 40\% of the work was for the termination proof,
