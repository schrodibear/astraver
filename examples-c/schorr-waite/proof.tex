...
to prove that the implementation of Figure~\ref{fig:algo} indeed
...
\item {I3} : means that for nodes initially unreachable, the mark
  remains unchanged.
\item {I4} : means all nodes different of null and
  reachable by root before the function are valid ({\huge utile ????
    preservation triviale ??? })
\item {I5a} : defines the backtracking stack as on Figure~\ref{fig:stack}.
\item {I5b} : means that all nodes in the \texttt{stack} are marked.
\item {I5c} : the trickest annotation, due to {\Huge ?}, means that
  ...
  in the stack.
\item {I5d} : means all nodes out of \texttt{stack} have
  the same children as before the function. 
\item {I5e} : a trick one also, that is essential for being able to
...
. validité des l'accès
. autres ?

with Simplify: 4/12

ajout axiome reachable(p,p) : 7/12
