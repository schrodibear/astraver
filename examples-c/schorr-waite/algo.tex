...
source code, is then given in Figure~\ref{fig:code}.This algorithm is
a non-recursive graph marking. In the Figure~\ref{fig:algo} we can see
two pointers t and p and 3 moves push,swing and pop. t is the current
state and p is used for backtracking. Push is used when t isn't
marked id when the subgraph are totally not explore, this move is
marked t and descends into it's left child. Swing is used the left
children of t are explored, this move is decends t into it's right
brother. Pop is used when the left and right children of t are
explored, this move go up in the graph.        
\begin{figure}[t]
\begin{center}
  \unitlength=0.5mm
...
\put(160,180){\rnode{null}{\makebox(0,0)[b]{NULL}}}
...
\end{center}
\caption{Schorr-Waite algorithm}
\label{fig:algo}
\end{figure}
