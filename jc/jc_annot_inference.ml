  ...
  match t.jc_term_node with
	...
	begin
	  match destruct_alloc t1 with
		...
		let offt = full_term_no_loc t3 integer_type dummy_region in
		vio, Some offt 
	end
    | _ -> assert false
...
(* 	    JCTmatch (term_of_expr e, ptl) *)
    in
      full_term_no_loc tnode e.jc_expr_type e.jc_expr_region 
  in
...
let collect_statement_asserts s = 
  match s.jc_statement_node with
    | JCSdecl(_,Some e1,_) | JCSassign_var(_,e1) 
    ...
    | JCSreturn_void | JCSreturn _ | JCSthrow _ | JCSlabel _ ->
	[]
...
let rec collect_targets filter_asrt targets s =
  let rec collect targets s =
    ...
    let targets = 
      (List.map (target_of_assertion s s.jc_statement_loc) asrts) @ targets 
    in
    match s.jc_statement_node with
      | JCSlabel(_, s) | JCSdecl(_,_,s) | JCScall(_,_,s) -> 
	  collect targets s
      | JCSblock sl ->
...
let set_equivalent_terms t1 t2 = 
  let tl = 
    try 
      Hashtbl.find !pointer_terms_table t2.jc_term_node
    with Not_found -> [] 
  in
  let tl = List.filter (fun t -> t <> t1) tl in
    Hashtbl.replace !pointer_terms_table t1.jc_term_node (t2 :: tl)
...
let assign_expr mgr pre t e =
  ...
  let forget_vars = Array.of_list forget_vars in
    ...
    begin
      try 
	let t1 = t in
	let t2 = Jc_pervasives.term_of_expr e in
	match t1.jc_term_type, t2.jc_term_type with
...
(*  let formal_vars =
  ...
  List.iter2
    (fun vi e -> let t = term_var_no_loc vi in
       assign_expr mgr pre t e)
    fi.jc_fun_info_parameters el;
...
and intern_ai_statement iaio abs curinvs s =
  ...
  begin match s.jc_statement_node with
	...
	begin match term_of_expr e1 with
	  | None -> () (* TODO *)
	  | Some t1 ->
	      let dereft = term_no_loc (JCTderef(t1,LabelHere,fi)) fi.jc_field_info_type in
	...
	let el = call.jc_call_args in
	  ...
	  in
		...
		let vars_writes = 
		  VarSet.fold
		    (fun vi acc -> Vai.all_variables (term_var_no_loc vi) @ acc)
		    fi_writes [] 
		in
...
let rec wp_statement weakpre = 
  ...
  fun target s curposts ->
    ...
    let curposts = match s.jc_statement_node with
      | JCSlabel(_,s) -> wp_statement weakpre target s curposts
      | JCSdecl(vi,eo,s) ->
...
let rec backprop_statement target s curpost =
  ...
  let curpost = match s.jc_statement_node with
    | JCSlabel(_,s) -> backprop_statement target s curpost
    | JCSdecl(vi,eo,s) ->
	 ...
	 in
	   match slo with
		 ...
		 if not (List.mem fi.jc_fun_info_tag !inspected_functions) then
		   record_ai_inter_annotations mgr iai fi loc fs)
	  fi.jc_fun_info_calls
