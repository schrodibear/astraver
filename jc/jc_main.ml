  ...
  if !Jc_options.separation_sem = SepRegions then begin
    ...
    StringHashtblIter.iter
      (fun _id data ->
         List.iter
           (function
            | Jc_typing.ABaxiom (pos, id, labs, a) ->
...
let main () =
  ...
  try
    ...
    | [file] ->
      ...
      Array.iteri
        (fun i l -> List.iter (fun fi -> fi.fun_component <- i) l)
        components;

      (* (optional) phase 5: inference of annotations *)
      if !Jc_options.annotation_sem <> AnnotNone then begin
             ...
             (fun _ (fi, r) (acc, regions) ->
                ...
                let acc =
                  if RegionSet.mem r regions then acc
                                             else Jc_interp.tr_region r acc
                in
    ...
    push_decls
      (StringHashtblIter.fold
         (fun _ ei acc -> Jc_interp.tr_exception ei acc)
    ...
    push_decls
      (IntHashtblIter.fold
         (fun _ (li, p) acc ->
            Jc_options.lprintf "Logic function %s@." li.li_name;
    ...
    push_decls
      (IntHashtblIter.fold
         (fun _ (f, loc, s, b) acc ->
            Jc_options.lprintf
              "Pre-treatement Function %s@." f.fun_name;
            Jc_interp.pre_tr_fun f loc s b acc)
         Jc_typing.functions_table);
    push_decls
      (IntHashtblIter.fold
         (fun _ (f, loc, s, b) acc ->
            Jc_options.lprintf "Function %s@." f.fun_name;
    ...
    if !Jc_options.inv_sem = InvOwnership then begin
    ...
    *)
    ...
    if !Jc_options.has_floats then  begin
