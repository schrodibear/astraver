...
\end{code}
where in the post-condition, $\old(e)$ means the value of expression
...
(pointer arithmetic), hence $(\verb|shift|~t~i)~\#~s$ then denotes
$t[i]$ is the memory state $s$. Dealing with pointer arithmetic in
...
tactics. However, this des not work immediately, because indeed we do
not known whether $i$ and $j$ are different. So the Coq may be made by
...
\}
\end{code}
In the pre-condition, the new predicate $\verb|valid_range|(t,0,n)$ is used
to specify that all memory locations $t[i]$ for $0\leq i < n$ are
valid. Notice also the use of universal quantification in the
post-condition. 
...
the first to do manually is to unfold the definition of
...
cells, the represents the values if the \verb|balance| field for each
...
subst; caduceus.
\end{verbatim}
for each subgoals. The situation is alomost the same for
\verb|withdraw|.
...
allocation map. This map is precisely telling if a given base address
corresponds to an allocated block and, if so, its length. The map
\texttt{a} changes whenever memory is allocated.
...
the first obligation is to show the validity os use of pointer
\verb|p1| in \verb|p1->balance = 0|, it is solves by 
\begin{verbatim}
unfold purse_inv; intuition.
\end{verbatim}
...
supposed to happen, and indeed the result of \verb|test1| would not be
...
\verb|p->balance|. So, the right thing to do is to add a so-called
...
\end{code}
Of course, that modification requires to make again the verification
of function \verb|credit|: the second obligation is a bit more
complicated, it is nevertheless solves by Simplify automatically. With
Coq, the proof must be completed also follows
...
result1 = 0
\end{verbatim}
Now the important hypothesis with have is \verb|H8|: it specifies that
between memory states \verb|balance0| and \verb|balance1|, the only
memory location that has been modified is the one pointed by
\verb|p2|. So, since we now that \verb|p1| and \verb|p2| are different
