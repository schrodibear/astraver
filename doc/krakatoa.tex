...
implementation, in the sense that it satisfies the specification
given.
...
formulas call \emph{verification conditions}, which express the
validity of the program. The generated formulas are then displayed
into the common graphical VC viewer of the Why platform, as shown in
Figure~\ref{fig:gwhy}
...
\begin{example}
  \texttt{(byte)1000} is $1000 \bmod 256$ \emph{i.e.} $232$.
\end{example}
...
Important: implicit casts from integers to Java integer type are forbidden.
...
\begin{itemize}
\item The caller must guarantee that the callee it is called in a
...
\subsection{Assertions}
  ...
  \item Additional clause:
    \begin{flushleft}\ttfamily\color{blue}
      //@ assert \textsl{prop}
    \end{flushleft}
  \item Specify that the given property is true at the corresponding
    program point
...
\subsection{Loop annotations}
  ...
  \begin{itemize}
  \item Additional clauses for loops:
    \begin{flushleft}\ttfamily\color{blue}
      /*@ loop invariant \textsl{prop} \\
      ~~@ loop assigns \textsl{tset} \\
      ~~@*/
    \end{flushleft}
  \item Specify that
    \begin{itemize}
...
\section{Data invariants}
  ...
  Are properties on data supposed to hold ``permanently''.
...
\subsection{Hybrid predicates}
  ...
  \begin{itemize}
  \item Logic functions and predicates may depend on C heap
