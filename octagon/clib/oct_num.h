...
#ifdef OCT_USE_TAG
static inline bool num_max_is_left(num_t* r, const num_t* a, const num_t* b)
{ bool left = (*a>=*b); *r = left ? *a : *b; return left; }

static inline bool num_min_is_left(num_t* r, const num_t* a, const num_t* b)
{ bool left = (*a<=*b); *r = left ? *a : *b; return left; }
#endif
...
{ 
  unsigned char *aa = (unsigned char*)a, *cc = (unsigned char*) c;
  int i;
  for (i=0;i<sizeof(num_t);i++) cc[sizeof(num_t)-i-1]=aa[i];
  return sizeof(num_t);
}

static inline size_t num_deserialize(num_t* a, void* c) 
{ 
  unsigned char *aa = (unsigned char*)a, *cc = (unsigned char*) c;
  int i;
  for (i=0;i<sizeof(num_t);i++) aa[sizeof(num_t)-i-1]=cc[i];
  return sizeof(num_t);
}
...
static inline int num_serialize_size(num_t* a) 
{ return sizeof(num_t); }
...
{ 
  bool left = 
    (!a->d) ? true : (!b->d) ? false : (a->n*b->d>=b->n*a->d) ? true : false;
  *r = left ? *a : *b; 
  return left; 
}
...
{ 
  bool left = 
    (!a->d) ? ((!b->d) ? true : false) 
    : (!b->d) ? true : (a->n*b->d<=b->n*a->d) ? true : false;
  *r = left ? *a : *b; 
  return left; 
}
#endif
...
{ 
  bool left;
  if (a->inf) { left=1; r->inf=1; }
  else if (b->inf) { left=0; r->inf=1; }
  else { 
    r->inf=0; 
    left = mpz_cmp(a->num,b->num)>0;
    mpz_set(r->num,left?a->num:b->num); 
  }
  return left;
}
...
{ 
  bool left;
  if (a->inf)
    if (b->inf) { left=1; r->inf=1; }
    else { left=0; r->inf=0; mpz_set(r->num,b->num); }
  else if (b->inf) { left = 1; r->inf=0; mpz_set(r->num,a->num); }
  else { 
    r->inf=0; 
    left = mpz_cmp(a->num,b->num)<0;
    mpz_set(r->num,left?a->num:b->num); 
...
{ 
  ...
  if (a->inf) { left=1; r->inf=1; }
    ...
    r->inf=0; 
    left=mpq_cmp(a->num,b->num)>0;
    mpq_set(r->num,left?a->num:b->num); 
  }
  return left;
}
...
{ 
  ...
  if (a->inf)
    if (b->inf) { left=1; r->inf=1; }
    else { left=0; r->inf=0; mpq_set(r->num,b->num); }
  else if (b->inf) { left=1; r->inf=0; mpq_set(r->num,a->num); }
  else { 
    r->inf=0; 
    left=mpq_cmp(a->num,b->num)<0;
    mpq_set(r->num,left?a->num:b->num); 
...
static inline bool num_max_is_left(num_t* r, const num_t* a, const num_t* b)
{ 
  bool left=mpfr_cmp(a->num,b->num)>0;
  mpfr_set(r->num,left?a->num:b->num,GMP_RNDU); 
  return left;
}
