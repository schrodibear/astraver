...
/*@ requires len >= 0 && 
  @ 	\valid(key) && \valid_range(d,0,256) &&
  @		\valid_range(indata,0,len) &&  \valid_range(outdata,0,len)	
  @*/
...
#define RC4_LOOP(a,b,i)	LOOP(a[i],b[i])
	...
	if (i)
		{
		/*@ invariant i>0 && i<=len && \valid_range(d,0,256) &&
		  @		(i>0 => \valid_range(indata,0,8) && \valid_range(outdata,0,8)) &&
		  @		 indata == \old(indata)+((len>>3L)-i)*8 && outdata == \old(outdata)+((len>>3L)-i)*8 
	 	  @ variant i
	      @*/	
		while(1) //for (;;)
			{
		...
		/*@ assert i==0 &&
		  @ 	(i==0 => indata == \old(indata)+((len>>3L)*8) && 
		  @				  outdata == \old(outdata)+((len>>3L)*8))
		  @*/
		
	i=(int)len&0x07;
	/*@ assert i== (len&0x07) && \valid_range(indata,0,i) &&
	  @				 \valid_range(outdata,0,i)
	  @*/
