...
Definition p3 (* validation *)
  ...
  := let (result, Post1) =
     ...
     | (Qval (exist result0 Post3)) =>
       let (result1, Post5) =
         let (result1, Post6) = (exist_1 [result1: Z]`result1 = 1` `2`
           Post3) in
...
Definition p4 (* validation *)
  ...
  := let (result, Post1) = (exist_1 [result: bool]result = true true
     ...
     | (left Test2) =>
         let (result0, Post7) =
           let (result0, Post8) = (exist_1 [result0: Z]`result0 = 1` 
             `1` (refl_equal ? `1`)) in
           (exist_1 (qcomb [result1: Z]`result1 = 1` [result1: unit]False) 
...
Definition p5 (* validation *)
  ...
  := let (result, Post3) =
       ...
       | (left Test2) =>
           let (result0, Post5) =
             let (result0, Post6) = (exist_1 [result0: Z]`result0 = 1` 
               `1` (refl_equal ? `1`)) in
             (exist_1 (qcomb [result1: Z]`result1 = 1` [result1: unit]False) 
...
Definition p6 (* validation *)
  ...
  := let (result, Post3) =
     ...
     | (Qval (exist result0 Post9)) =>
       let (result1, Post11) =
         (exist_1 (qcomb [result1: unit]True [result1: unit]False) (Exn unit
                                                                    tt) I) in
       Cases (decomp1 Post11) of
       | (Qval (exist result2 Post12)) =>
  ...
  := [x: Z]
       ...
       | (Qval (exist result1 Post4)) =>
         let (x1, result2, Post2) =
           let (result2, Post2) = (exist_1 [result2: Z]result2 = `2` 
             `2` (refl_equal ? `2`)) in
  ...
  := let (result, Post1) =
       ...
       | (Qval (exist result0 Post3)) =>
         let (result1, Post5) =
           ...
           (exist_1 (qcomb [result2: Z]`3 = 2` [result2: unit]`1 = 2`) 
           (Exn unit result1) Post6) in
         Cases (decomp1 Post5) of
         | (Qval (exist result2 Post7)) =>
           let (result3, Post9) = (exist_1 [result3: Z]`result3 = 2` 
             `1` Post7) in
     ...
     | (Qval (Qexn result0 Post13)) =>
       let (result1, Post14) = (exist_1 [result1: Z]`result1 = 2` `3`
         Post13) in
...
Definition p13 (* validation *)
  ...
  := [x: Z]
       ...
       | (Qval (Qval (exist result0 Post12))) => (exist_2 [x1: Z]
         [result1: unit]`x1 = 2` x0 result0 Post12)
       | (Qexn _ Post13) =>
  ...
  := [x: Z]
       ...
       | (Qval (exist result0 Post10)) =>
         let (result1, Post12) =
           ...
           (Cases (btest
                   [result1:bool](if result1 then `x = 2` else `x <> 2`)
                   result1 Bool2) of
           | (left Test4) =>
               let (result2, Post15) =
         ...
         | (Qval (exist result2 Post18)) =>
           let (result3, Post20) =
             ...
             (Cases (btest
                     [result3:bool](if result3 then `x = 3` else `x <> 3`)
                     result3 Bool3) of
             | (left Test6) =>
                 let (result4, Post23) =
             ...
             | (right Test5) =>
                 let (result4, Post22) = (exist_1 [result4: unit]`x <> 1` /\
                   `x <> 2` /\ `x <> 3` tt
                   (p14_po_3 x Post10 Post18 Test5)) in
                 (exist_1 (qcomb [result5: unit]`x = 3` [result5: unit]
                           `x <> 1` /\ `x <> 2` /\ `x <> 3`) (Val unit
                                                               result4)
                 Post22) end) in
           ...
           | (Qval (exist result4 Post26)) =>
             let (result5, Post28) =
               (exist_1 (qcomb [result5: unit]`x <> 1` /\ `x <> 2` /\
                         `x <> 3` [result5: unit]False) (Exn unit tt)
               Post26) in
             Cases (decomp1 Post28) of
             | (Qval (exist result6 Post29)) =>
               (exist_1 (qcomb [result7: unit]`x <> 1` /\ `x <> 2` /\
                         `x <> 3`
               ...
               (False_ind ? Post29))
             | (Qexn _ Post30) =>
               (exist_1 (qcomb [result6: unit]`x <> 1` /\ `x <> 2` /\
                         `x <> 3`
           ...
           | (Qexn _ Post27) =>
             (exist_1 (qcomb [result4: unit]`x <> 1` /\ `x <> 2` /\ `
                       x <> 3`
