...
Definition loop2 := (* validation *)
  [x: Z; Pre4: `x <= 10`]
        ...
        [Variant1: Z; wf1: (Variant2: Z)(Pre1: (Zwf `0` Variant2 Variant1))
         ...
         Pre3: Variant1 = `10 - x0`; Pre2: `x0 <= 10`]
          let (result, Bool1) =
            ...
            (exist_1 [result2: bool]
            (if result2 then `x0 < 10` else `x0 >= 10`) result1 Post6) in
          (Cases (btest
    ...
    let (x1, result0, Post9) =
      let (result0, Bool2) =
        let (result2, Post10) = (Z_gt_le_bool x0 `0`) in
        (exist_1 [result3: bool]
        (if result3 then `x0 > 0` else `x0 <= 0`) result2 Post10) in
      (Cases (btest [result0:bool](if result0 then `x0 > 0` else `x0 <= 0`)
              result0 Bool2) of
      | (left Test4) =>
          let (x1, result1, Post12) =
