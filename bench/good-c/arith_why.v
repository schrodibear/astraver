...
Definition test (* validation *)
  : (k: Z)(i: Z)(j: Z)
    (sig_3 Z Z unit [i0: Z][j0: Z][result: unit](`i0 = j + k` /\
     `j0 = 3 * j + 11 * k`))
  := [k: Z; i: Z; j: Z]
       ...
       let (i0, j0, l0, result0, Post5) =
         let (i0, result0, Post1) =
           let (result0, Post1) = (exist_1 [result0: Z]
             result0 = `j + k` `j + k` (refl_equal ? `j + k`)) in
         ...
         let (j0, result2, Post3) =
           ...
           (exist_2 [j1: Z][result3: unit]
           j1 = `j + (l0 + 10 * k + i0)` result2 tt Post3) in
