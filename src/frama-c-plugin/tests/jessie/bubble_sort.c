...
/*@ 
  inductive Permut{L1,L2}(int* a, integer index_start, integer end_start) 
  { 
  case Permut_refl{L}:  
  \forall int* a, integer index_start, end_start;
  Permut{L,L}(a, index_start, end_start); 
  
  case Permut_sym{L1,L2}:  
  \forall int* a, integer index_start, end_start;  
  Permut{L1,L2}(a, index_start, end_start) ==> 
  Permut{L2,L1}(a, index_start, end_start); 
  
  case Permut_trans{L1,L2,L3}:  
  \forall int* a, integer index_start, end_start;  
  Permut{L1,L2}(a, index_start, end_start) && 
  Permut{L2,L3}(a, index_start, end_start) ==>  
  Permut{L1,L3}(a, index_start, end_start); 
...
/*@ 
  ...
  */ 
void swap(int t[], int i, int j, int n) { 
  int tmp = t[i]; 
  t[i] = t[j]; 
  t[j] = tmp; 
} 
...
/*@
  ...
  behavior permutation: 
		ensures Permut{Old,Here}(a, 0, length-1);
*/
void bubble_sort(int* a, int length) 
{
   ...
   // loop assigns a[0..up-1];
		...
		for permutation:
			loop invariant Permut{Pre,Here}(a, 0, length-1);
	*/
	for (; up < length; up++, fixed_up = up)   
	{   
		//@ for sorted: assert sorted(a, 0, up-1);				
    //@ for permutation: assert Permut{Pre,Here}(a, 0, length-1);
		fixed_up = up;
		down=up; 
