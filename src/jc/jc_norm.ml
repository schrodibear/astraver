...
let goto_exception_for_label lab =
  ...
  if lab = "return_label" then return_label
  else
    try
...
let normalize_switch pos e caselist =
  ...
  let rec cannot_fall_trough e =
    match e#node with
    | JCPEblock [] -> false
    | JCPEblock elist -> cannot_fall_trough List.(hd (rev elist))
            ...
            [mktry ~pos
               ~expr:
                 (mkwhile ~pos ~behaviors:behs ?variant:var
                    ...
                    ())
               ~catches:[mkcatch ~pos ~name:(tmp_var_name()) ~exn:loop_exit ()]
               ()
              ])
    ()
...
let duplicable =
  Iterators.IPExpr.fold_left
    (fun acc e -> acc &&
                  ...
                  | JCPEbinary _ | JCPEcast _ | JCPEreinterpret_cast _ | JCPEsubtype _ | JCPEbase_block _ -> true
                  ...
                  | JCPEswitch _ | JCPEcontract _ -> false)
    true
...
let rec in_label_tree lab =
  function
  | LabelItem l -> l=lab
  | LabelBlock l -> in_label_tree_list lab l

and in_label_tree_list lab =
  function
  | [] -> false
  | h :: r ->
...
let rec goto e lz =
    ...
    | JCPEblock el ->
      ...
      let el, _ =
        List.fold_left (fun (acc,lz1) e1 ->
          let e2,lz2 = goto e1 lz1 in e2 :: acc, lz2) ([],lz1)
          el
      in
  ...
  in
  new pexpr_with ~node:enode e, lz2
...
let rec expr e =
  let enode =
    ...
    | JCPEcast_mod (e, id) -> JCNEcast_mod (expr e, id)
