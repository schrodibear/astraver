...
(*i $Id: util.ml,v 1.144 2008-02-01 12:55:27 marche Exp $ i*)
...
(* [cook_loop_invariant lab loc] checks whether the loop invariant
   labelled with [lab] is associated to a formula in the locs file
   ...
   is.
*)
let cook_loop_invariant lab (lab_split : string option) =
  try 
  ...
  with Not_found -> 
    ...
    "", dummy_reloc (* Loc.extract loc *)

(* [cook_explanation lab e] processes the raw explanation [e]
   generated by the WP calculus, which refers to the Why file, and
   produces a better explanation depending on the locs file given on
   the command line: if the raw explanation has a label, it is check
   whether this label is present in the locs file, and if it is the
   case, replaces the location in the Why file by the location given
   there, and also if a precise kind of VC is given, produces a
   refined explanation.
*)
let cook_explanation (lab_split : string option) e =
  let e,l =
    match e with
      ...
      | VCEinvinit(lab,p) -> 
	  let s,loc  = cook_loop_invariant lab lab_split in
	  EKLoopInvInit s, loc 
      | VCEinvpreserv(lab,p) -> 
	  let s,loc  = cook_loop_invariant lab lab_split in
