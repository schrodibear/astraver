    ...
    match formula with 
	  ...
	  if (keep_quantification_link_beween_vars) then
	    begin
	      ...
	      (* inner_vars := VarStringSet.add bv !inner_vars; 
		 let l' = Tvar(id')::tl in *)
	      ac_fv_set:=(VarStringSet.add bv !ac_fv_set);  (* Mise � jour de la liste des variables fraiches *)
	...
	match ctx with
	  ...
	  | (p_cnf,Dpredicate_def (loc, ident, def)) :: l ->
		...
		if abstr_subset_of_pdl preds_of_p_cnf relevantPreds then 
		  ...
		  begin
		    (* Lorsqu'un Pr�dicat est retir�, sa signature est laiss�e sous la forme d'un logic *)
...
(* A disparu dans v1.49 de YM *)		
	  | (p_cnf,Daxiom (loc, ident, ps)) :: l-> 
	      (* Spliter le pr�dicat de d�finition en Clauses devenant autant d'axiomes *)
		...
		if (abstr_subset_of_pdl preds_of_p_cnf relevantPreds) then 
		  ...
		  begin
		    ...
		    (* Ensuite, on test chacune des clauses pour �ventuellement les pr�server *)
		    let p_list=(split_one [] 1 p_cnf) in
		    let ax_list=List.map (fun p -> 
		      let i=my_fresh_hyp_str () in
	  ...
	  | (p_cnf,Daxiom (loc, ident, ps)) :: l-> 
		...
		if (abstr_subset_of_pdl preds_of_p_cnf relevantPreds)  then 
		  ...
		  begin
		    (* Sinon, on splite le pr�dicat de d�finition en Clauses devenant autant d'axiomes *)
		    let p_list=(split_one [] 1 p_cnf) in
		    let ax_list=List.map (fun p -> 
		      let i=my_fresh_hyp_str () in
