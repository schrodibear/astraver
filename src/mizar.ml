...
(*i $Id: mizar.ml,v 1.1 2003-09-17 15:48:47 filliatr Exp $ i*)
...
let rec print_term fmt = function
  ...
  | Tapp (id, [a; b; c]) when id == store ->
      fprintf fmt "(@[%a+*(%a,@ %a)@])" 
	print_term b print_term a print_term c
...
let rec print_predicate fmt = function
  ...
  | Papp (id, tl) -> 
      fprintf fmt "@[%a(%a)@]" Ident.print id print_terms tl
  | Pimplies (_, a, b) ->
      fprintf fmt "(@[%a implies@ %a@])" print_predicate a print_predicate b
  | Pif (a, b, c) ->
