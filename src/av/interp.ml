...
let bin_op ~e : [< bin_op] * operator_type -> _ =
  ...
  | `Beq | `Bneq | `Bsub as op', `Pointer ->
    ...
    | `Bsub -> Op (O.F.Core.pointer "sub_pointer", Any)
    end
  (* reals *)
  | `Bgt | `Blt | `Bge | `Ble | `Beq | `Bneq |
    `Badd | `Bsub | `Bmul | `Bdiv | `Bmod as op',
  ...
  fun ty_dst ty_src form ->
    ...
    fun ?(modulo=false) ~e ~e1 e' ->
      ...
      | Ty (Numeric (Real (Float _ as f))), Ty (Numeric (Real Real)) ->
        ...
        | Expr, { expr_node = App (User (_, "of_real_exact"),
                                   Cons ({ expr_node = Const (Real _); _ } as c, Nil), _); _ }
          -> c
        | _ -> apply' @@ Of_float f
        end
...
let rec term :
  ...
  let t' =
    ...
    | JCTapp { app_fun = { li_name = "\\integer_min" | "\\integer_max" as op; _ }; app_args = [t1; t2]; _ } ->
      let ft = ft (Ty O.Ty.integer) in
      return @@ O.(if op = "\\integer_min" then T.min else T.max) (ft t1) @@ ft t2
    | JCTapp { app_fun = { li_name = "\\integer_abs"; _ }; app_args = [t]; _ } ->
...
let[@warning "-32"] term_minus_one = term_num "-1"
...
let rec predicate ~type_safe ~global_assertion ~relocate lab oldlab p =
  ...
  let p' =
    ...
    | JCAallocable (lab', t) ->
      ...
      let acr = ac, t#region in
      let t = ft Any t in
      Interp_struct.(fresh ~for_:(`alloc_tables_in (`fresh label)) ~in_param acr pc t O.T.(int 0))
    | JCAbool_term t1 ->
...
let rec mark_predicate ?(recursively=true) ~e ?kind p =
  let mark_predicate' =
    function
    | (Labeled ({ l_kind; l_pos; _ } as l, p) : pred) ->
      (Labeled ({ l with
...
let rec collect_locations ~type_safe ~global_assertion ~in_clause before loc (refs, mems) =
  ...
  | JCLderef_term (t1, fi) ->
    ...
    let iloc = O.T.(F.Core.pset "pset_singleton" $. t1') in
...
let rec collect_pset_locations t ~type_safe ~global_assertion lab loc =
  ...
  | JCLvar ({ vi_type = JCTpointer _; _ } as vi)  ->
  ...
  | Some (pos, locs) ->
    ...
    MemoryMap.fold
      (fun (mc, r) pes acc ->
         ...
         in
         let mem_assigns = P.locate ~p ~kind:JCVCassigns @@ App (O.F.Core.assigns "not_assigns", args) in
         O.P.(acc && mem_assigns))
      mems
...
let loop_assigns ~type_safe ?region_list before ef =
  Option.map_default
    ~default:True
    ~f:(fun locs -> assigns ~type_safe ?region_list before ef @@ Some (Loc.dummy_position, locs))
  ...
  let shift, upd =
    ...
    if safety_checking () then
      ...
      (if P.(off = Int_offset 0) then
         ...
         E.locate ~e ~kind:JCVCpointer_shift (O.F.Core.shift_safe "shift_" $ tt ^ var tmpp ^ tag_id ^. var tmpi)
       else
         O.F.Core.shift_safe "shift_typesafe" $ var tmpp ^. var tmpi),

      if P.(off = Int_offset 0) then
        E.locate ~e ~kind:JCVCpointer_deref (O.F.Core.upd_safe "upd" $ alloc ^ var mem ^ var tmpp ^. var tmp2)
      else if not typesafe then
        E.locate ~e ~kind:JCVCpointer_deref
          (O.F.Core.upd_offset_safe "upd_offset" $ alloc ^ tt ^ var mem ^ var tmpp ^ tag_id ^ var tmpi ^. var tmp2)
      else
        E.locate ~e ~kind:JCVCpointer_deref
          (O.F.Core.upd_offset_safe "upd_offset_typesafe" $ alloc ^ var mem ^ var tmpp ^ var tmpi ^. var tmp2)
    else
      (if P.(off = Int_offset 0) then
         var tmpp
       else
         O.F.Core.shift_unsafe "shift_" $ var tmpp ^. var tmpi),
      O.F.Core.upd_unsafe "upd" $ var mem ^ var tmp1 ^. var tmp2
  in
...
and make_deref_simple ~e e1 fi =
  ...
  if safety_checking () then
    ...
    | (_, (Int_offset s as off), Some lb, Some rb), false when bounded lb rb s ->
      E.locate ~e ~kind:JCVCpointer_deref
        (O.F.Core.acc_offset_safe "acc_offset_bounded" $ tt ^ mem ^ expr e1 ^ tag_id ^. offset off)
    | (_, Int_offset s, Some lb, Some rb),        true when bounded lb rb s ->
      E.locate ~e ~kind:JCVCpointer_deref
        (O.F.Core.acc_safe "acc_safe" $ mem ^. expr e1)
    | (p, (Int_offset s as off), Some lb, _), false when lbounded lb s ->
      E.locate ~e ~kind:JCVCpointer_deref_bounds
        (O.F.Core.acc_offset_safe "acc_offset_lbounded" $ tt ^ alloc ^ mem ^ expr p ^ tag_id ^. offset off)
    | (p, (Int_offset s as off), Some lb, _), true when lbounded lb s ->
      E.locate ~e ~kind:JCVCpointer_deref_bounds
        (O.F.Core.acc_offset_safe "acc_offset_lbounded_typesafe" $ alloc ^ mem ^ expr p ^. offset off)
    | (p, (Int_offset s as off), _, Some rb), false when rbounded rb s ->
      E.locate ~e ~kind:JCVCpointer_deref_bounds
        (O.F.Core.acc_offset_safe "acc_offset_rbounded" $ tt ^ alloc ^ mem ^ expr p ^ tag_id ^. offset off)
    | (p, (Int_offset s as off), _, Some rb), true when rbounded rb s ->
      E.locate ~e ~kind:JCVCpointer_deref_bounds
        (O.F.Core.acc_offset_safe "acc_offset_rounded_typesafe" $ tt ^ alloc ^ mem ^ expr p ^. offset off)
    | (p, Int_offset 0, None, None), _ ->
      E.locate ~e ~kind:JCVCpointer_deref
        (O.F.Core.acc_safe "acc"  $ alloc ^ mem ^. expr p)
    | (p, off, _, _), false ->
      E.locate ~e ~kind:JCVCpointer_deref
        (O.F.Core.acc_offset_safe "acc_offset" $ alloc ^ tt ^ mem ^ expr p ^ tag_id ^. offset off)
    | (p, off, _, _), true ->
      E.locate ~e ~kind:JCVCpointer_deref
        (O.F.Core.acc_offset_safe "acc_offset_typesafe" $ alloc ^ mem ^ expr p ^. offset off)
  else
    O.F.Core.acc_unsafe "acc" $ mem ^. expr Any e1
...
and make_reinterpret ~e e1 st =
  ...
  let op =
    let from_bitsize, to_bitsize = Pair.map (fun { fi_bitsize = s; _ } -> s) (fi_from, fi_to) in
           ...
           (fun ps ->
              let omin_omax =
                let app f =
                  let open O.T in
                  function
                  | `Old -> O.F.Core.alloc_table f $ at' before alloc ^. p
                  | `New -> O.F.Core.alloc_table f $ at' LabelHere alloc ^. ps
                in
              ...
              let pred_names =
                let enum_name =
                  function
                  | { fi_type = JCTenum { ei_type; _ }; _ } -> string_of_some_enum ei_type
  ...
  let not_assigns_assumption =
    O.P.conj
      O.T.[F.Core.rmem "rmem" $. mem `Old = mem `New;
           F.Core.rmem "rfactor" $. mem `Old = int c;
           F.Core.rmem "rpointer_new" $ mem `Old ^. e' = sidecast ~r:e1#region e' s_to]
  in
  ...
  let e' =
    ...
    | JCEbinary (e1, ((`Beq | `Bneq as o), `Pointer), e2) when safety_checking () ->
      ...
      if is_null e1 && is_null e2 then
        ...
        in
        let th =
          if safety_checking () then O.F.Core.eq_pointer_safe
          else O.F.Core.eq_pointer_unsafe
        in
        E.locate ~e @@
        O.E.(th (match o with `Beq -> "eq_pointer" | `Bneq -> "neq_pointer") $ at1 ^ at2 ^ e1' ^. e2')
    | JCEbinary (e1, (`Bsub, `Pointer), e2) ->
      let th = O.F.Core.(if safety_checking () then sub_pointer_safe else sub_pointer_unsafe) in
    ...
    | JCEshift (e1, e2) ->
      begin match
        match e1#typ with
        | JCTpointer (JCtag ({ si_final = true; _ }, []), _, _) -> None
        | JCTpointer (JCtag ({ si_name; _ } as st, []), _, _) ->
      ...
      | Some (tt, tag') when safety_checking () ->
        O.E.(O.F.Core.shift_safe "shift_" $ tt ^ expr Any e1 ^ tag' ^. expr (Ty O.Ty.integer) e2)
      | None when safety_checking() ->
        O.E.(F.Core.shift_safe "shift_typesafe" $ expr Any e1 ^. expr (Ty O.Ty.integer) e2)
      | _ ->
        O.E.(F.Core.shift_unsafe "shift_" $ expr Any e1 ^. expr (Ty O.Ty.integer) e2)
      end
    | JCEif (e1, e2, e3) ->
    ...
    | JCEinstanceof (e1, st) ->
      let tt = tag_table_var (struct_root st, e1#region) in
      (* always safe *)
      O.E.(F.Core.instanceof "instanceof"  $ tt ^ expr Any e1 ^. tag st)
    | JCEdowncast (e1, st) ->
      let tt = tag_table_var (struct_root st, e1#region) in
      ...
      then expr Any e1
        ...
        O.E.(
          F.Core.(if safety_checking () then (fun args -> E.locate ~e (downcast_safe downcast $ args))
    ...
    | JCEsidecast (e1, st) ->
      ...
      O.E.(
        (if safety_checking () then
           let sidecast_mod =
             F.Core.(if infunction.fun_effects.fe_reinterpret then sidecast_safe_reinterpret else sidecast_safe)
           in
           (fun args -> E.locate ~e (sidecast_mod sidecast $ args))
         else
           ($) (F.Core.sidecast_unsafe sidecast))
          (tt ^ expr Any e1 ^. tag st))
             ...
             | Some li ->
               ...
               | _ ->
                 Options.jc_error
                   ...
                   li.li_name
           in
           let pre = O.P.(O.F.Core.zwf r $ this_measure_why ^. cur_measure_why) in
           fun call ->
             E.locate
               ~e
               ~kind:JCVCvar_decr
...
let global_imports ~for_ ~lookup =
  ...
  fun e ->
    ...
    VarMap.fold (function { vi_type = JCTpointer _; _ } -> skip | _ -> Fn.const @@ cons_global_import None) e.e_globals |>
    match for_ with
    | `Logic -> Fn.id
    | `Code ->
...
let prop pos id ~is_axiom labels p =
  ...
  let pos = Position.of_pos pos in
  O.[Entry.some @@
...
let map_embedded_fields ~f x =
  ...
  | JCTpointer (JCtag (st, _), _, _) ->
    List.concat_map
      st.si_fields
      ~f:(function
        | { fi_type = JCTpointer (_, Some l, Some r) as typ; _ } as fi ->
          f ~typ ~l ~r fi
        | _ -> [])
  | _ -> []
    ...
    (fun ~add ->
       StringHashtblIter.iter
         (fun _ (mc, r) ->
            add
              ((match mc with
               | JCmem_field { fi_struct; _ } -> Some (JCtag (fi_struct, []))
