...
and expr_node : type a. entry:_ -> safe:_ -> bw_ints:_ -> consts:_ -> _ -> a expr_node -> _ =
  ...
  | While ({ expr_node = Const (Bool true); _ }, inv, var, e2) ->
    pr
        ...
        let rec continuation
          ...
          fun ~state ~consts ~entry ~entry' ->
            ...
            | Entries ->
              ...
              | Theory ({ why_name = name; _ }, Some (deps, decls)) ->
                List.fold_left
                  (fun acc ->
                     function
                     | Use (_, Theory (id', _))
                ...
                if fst (H.find state name) = init then
                  ...
                  consts
              | Module ({ why_name = name; _ }, Some (deps, safe, decls)) ->
                List.fold_left
                  (fun acc ->
                     fun (Dependency d) ->
                       let enter (type k) =
            ...
            | Decls ->
              ...
              | Theory (_, None) -> consts
              | Module (_, None) -> consts
              | Theory ({ why_name = name; _ }, Some (_, decls)) ->
              ...
              | Module ({ why_name = name; _ }, Some (_, safe, decls)) ->
...
let file fmttr file =
  ...
  list
    ...
    (List.sort
       (fun e1 e2 ->
          let compare
              { why_name = id1; why_pos = pos1; _ }
              { why_name = id2; why_pos = pos2; _ } =
