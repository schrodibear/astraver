  ...
  map_si ac
    ~f:(fun si ->
      ListLabels.map
        si.si_fields
        ~f:(function
          | { fi_type = JCTpointer (fpc, Some fa, Some fb); _ } as fi ->
...
let valid_pred ~in_param ~equal ?(left=true) ?(right=true) ac pc =
  ...
  let validity =
    let omin, omax, super_valid =
      match pc with
      | JCtag ({ si_parent = Some(st, pp); _ }, _) ->
        let super_valid =
          let open O.T in
...
let instanceof_pred ~exact (type t1) (type t2) :
  ...
  let self_instanceof p = map_si ac pc ~f:(fun si -> [pred ~code:false p si]) in
      ...
      map_si ac pc
        ~f:(fun si ->
          ListLabels.map
            si.si_fields
            ~f:(function
              | ({ fi_type = JCTpointer (fpc, Some l, Some r); fi_bitoffset; _ } as fi)
    ...
    let assc =
      ...
      let name_of_x ac =
        ...
        | `tag_tables ->
          match ac with
          | JCalloc_bitvector ->
            Options.jc_error Loc.dummy_position "Unsupported alloc_struct frame conditions for bitvector regions"
          | JCalloc_root ri ->
       ...
       (function
         ...
         | (xt, p, _) :: ps ->
           ...
           make_predicates f xt
             [let pset_singleton p = O.T.(some ((Name.Theory.Core.pset, "pset_singleton") $.. [p])) in
              List.fold_left
                (fun acc (_, p, _) ->
                   O.T.(some ((Name.Theory.Core.pset_union, "pset_union") $.. [acc; pset_singleton p])))
                (pset_singleton p)
                ps]
...
let alloc_param : type t1 t2.
  ...
  let write_effects = List.map (function (Expr { expr_node = Var n; _ }, _ty') -> n |  _ -> error ()) writes in
  let params =
    ...
    |>
    List.map (function (Expr { expr_node = Var n; _ }, ty') -> (n, ty') |  _ -> error ())
  in
    ...
    let deps =
      ...
      Name.(
        if si.si_name = voidp then
          use_th Theory.Core.voidp_tag_id
        else if si.si_name = charp then
          use_th Theory.Core.charp_tag_id
        else
          [])
    in
    O.[Entry.some @@
...
let root ri =
  ...
  let deps =
    if ri.ri_name = Name.voidp then
      [Use (`Import None, O.Th.dummy @@ fst Name.Theory.Core.voidp)]
    else
      []
  in
  O.[Entry.some @@
...
let valid_pre ~in_param all_effects (* vi *) =
  function
  | { vi_type = JCTpointer (pc, lo, ro); vi_region; _ } as vi
...
let instanceof_pre all_effects (* vi *) =
  function
  | { vi_type = JCTpointer (pc, lo, ro) as vi_type; vi_region; _ } as vi
    when
