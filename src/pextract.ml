...
(* $Id: pextract.ml,v 1.1 2001-08-15 21:08:53 filliatr Exp $ *)
...
type rename_struct = { rn_map : Ident.t IdMap.t;
		       rn_avoid : Ident.t list }
...
let initialize id com =
  ...
  try
    ...
    let ety = match v with 
      | Ref (PureType c) -> c | Array (_,PureType c) -> c
      | _ -> raise Not_found 
    in
